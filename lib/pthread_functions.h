...
static __inline__
int get_affinity(pthread_t thread, int **cpus)
{
	cpu_set_t cpuset;
	int i, j, count=0;
	...
	for (i=0;i<CPU_SETSIZE;i++)
		if (CPU_ISSET(i, &cpuset))
			count++;
	*cpus = (int *) malloc(count * sizeof(**cpus));
	for (i=0,j=0;i<CPU_SETSIZE;i++)
		if (CPU_ISSET(i, &cpuset))
			(*cpus)[j] = i;	    // Note: '*cpus[j]' is interpreted as '*(cpus[j])'.
	return count;
}


#define print_affinity(tid, format, ...)                                                  \
do {                                                                                      \
	pthread_t __tid = tid;                                                            \
	const char * __format = format;                                                   \
	char __str[10*1024];                                                              \
	int * __cpus;                                                                     \
	int __i, __j=0,  __count=0;                                                       \
	                                                                                  \
	__count = get_affinity(__tid, &__cpus);                                           \
	__j += snprintf(__str + __j, sizeof(__str), __format, ##__VA_ARGS__);             \
	__j += snprintf(__str + __j, sizeof(__str), " affinity=[");                       \
	for (__i=0;__i<__count;__i++)                                                     \
		__j += snprintf(__str + __j, sizeof(__str), "%d, ", __cpus[__i]);         \
	__j -= 2;                                                                         \
	snprintf(__str + __j, sizeof(__str), "]\n");                                      \
	printf("%s", __str);                                                              \
	free(__cpus);                                                                     \
} while (0)
