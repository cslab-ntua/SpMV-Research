...
{
	return array_windowed_composite_metric(A, N, window_len, reduce_add, get_window_min_ctz_as_double) / N;
}
...
csr_value_features(char * title_base, __attribute__((unused)) _TYPE_I * row_ptr, __attribute__((unused)) _TYPE_I * col_idx, _TYPE_V * values, __attribute__((unused)) long m, __attribute__((unused)) long n, long nnz, int do_plot)
{
	...
	double vals_min, vals_max, vals_avg, vals_std, vals_ctz_avg, vals_ctz_std, vals_grouped_ctz_avg;
	...
	vals_avg = array_mean(vals, nnz);
	vals_std = array_std(vals, nnz, vals_avg);
	vals_ctz_avg = array_mean(vals, nnz, get_double_trailing_zeros);
	vals_ctz_std = array_std(vals, nnz, vals_ctz_avg, get_double_trailing_zeros);
	array_min_max(vals, nnz, &vals_abs_min, NULL, &vals_abs_max, NULL, get_double_abs);
	vals_abs_avg = array_mean(vals, nnz, get_double_abs);
	vals_abs_std = array_std(vals, nnz, vals_abs_avg, get_double_abs);
	array_min_max(vals, nnz, &vals_exp_min, NULL, &vals_exp_max, NULL, get_double_exponent);
	vals_exp_avg = array_mean(vals, nnz, get_double_exponent);
	vals_exp_std = array_std(vals, nnz, vals_exp_avg, get_double_exponent);
	vals_unique_num = array_unique_num(vals, nnz);
	vals_sgnexp_unique_num = array_unique_num(vals, nnz, get_double_upper_12_bits);
	if (do_plot)
	{
	...
	{
		vals_grouped_ctz_avg = array_mean_of_windowed_ctz_min(vals, nnz, ctz_group_size);
	...
	double * vals_diff;
	double vals_diff_min, vals_diff_max, vals_diff_avg, vals_diff_std, vals_diff_ctz_avg, vals_diff_ctz_std, vals_diff_grouped_ctz_avg;
	...
	vals_diff_avg = array_mean(vals_diff, nnz);
	vals_diff_std = array_std(vals_diff, nnz, vals_diff_avg);
	vals_diff_ctz_avg = array_mean(vals_diff, nnz, get_double_trailing_zeros);
	vals_diff_ctz_std = array_std(vals_diff, nnz, vals_diff_ctz_avg, get_double_trailing_zeros);
	array_min_max(vals_diff, nnz, &vals_diff_abs_min, NULL, &vals_diff_abs_max, NULL, get_double_abs);
	vals_diff_abs_avg = array_mean(vals_diff, nnz, get_double_abs);
	vals_diff_abs_std = array_std(vals_diff, nnz, vals_diff_abs_avg, get_double_abs);
	array_min_max(vals_diff, nnz, &vals_diff_exp_min, NULL, &vals_diff_exp_max, NULL, get_double_exponent);
	vals_diff_exp_avg = array_mean(vals_diff, nnz, get_double_exponent);
	vals_diff_exp_std = array_std(vals_diff, nnz, vals_diff_exp_avg, get_double_exponent);
	if (do_plot)
	{
	...
	for (ctz_group_size=4;ctz_group_size<=16;ctz_group_size+=4)
	{
		vals_diff_grouped_ctz_avg = array_mean_of_windowed_ctz_min(vals_diff, nnz, ctz_group_size);
	...
	free(vals_diff);
		...
		abs_vals_diff_avg = array_mean(abs_vals_diff, nnz);
		abs_vals_diff_std = array_std(abs_vals_diff, nnz, abs_vals_diff_avg);
		abs_vals_diff_ctz_avg = array_mean(abs_vals_diff, nnz, get_double_trailing_zeros);
		abs_vals_diff_ctz_std = array_std(abs_vals_diff, nnz, abs_vals_diff_ctz_avg, get_double_trailing_zeros);
		array_min_max(abs_vals_diff, nnz, &abs_vals_diff_abs_min, NULL, &abs_vals_diff_abs_max, NULL, get_double_abs);
		abs_vals_diff_abs_avg = array_mean(abs_vals_diff, nnz, get_double_abs);
		abs_vals_diff_abs_std = array_std(abs_vals_diff, nnz, abs_vals_diff_abs_avg, get_double_abs);
		if (do_plot)
		{
			double * running_avg;
		...
		for (ctz_group_size=4;ctz_group_size<=16;ctz_group_size+=4)
		{
			abs_vals_diff_grouped_ctz_avg = array_mean_of_windowed_ctz_min(abs_vals_diff, nnz, ctz_group_size);
		...
		dist_from_windowed_avg_abs_avg = array_mean(dist_from_windowed_avg, nnz, get_double_abs);
		dist_from_windowed_avg_abs_std = array_std(dist_from_windowed_avg, nnz, dist_from_windowed_avg_abs_avg, get_double_abs);
		dist_from_windowed_avg_ctz_avg = array_mean(dist_from_windowed_avg, nnz, get_double_trailing_zeros);
		dist_from_windowed_avg_ctz_std = array_std(dist_from_windowed_avg, nnz, dist_from_windowed_avg_ctz_avg, get_double_trailing_zeros);
		if (do_plot)
		{
			struct Figure * fig;
			struct Figure_Series * s;
		...
		for (ctz_group_size=4;ctz_group_size<=16;ctz_group_size+=4)
		{
			dist_from_windowed_avg_grouped_ctz_avg = array_mean_of_windowed_ctz_min(dist_from_windowed_avg, nnz, ctz_group_size);
	...
	double ctz_corrected, ctz_corrected_max;
	...
	for (k=4;k<=14;k++)
	{
		...
		vals_diff_sorted_window_abs_avg = array_mean(vals_diff_sorted_window, nnz, get_double_abs);
		vals_diff_sorted_window_abs_std = array_std(vals_diff_sorted_window, nnz, vals_diff_sorted_window_abs_avg, get_double_abs);
		vals_diff_sorted_window_ctz_avg = array_mean(vals_diff_sorted_window, nnz, get_double_trailing_zeros);
		ctz_corrected = vals_diff_sorted_window_ctz_avg - k - 6;
		if (ctz_corrected > ctz_corrected_max)
		{
			ctz_corrected_max = ctz_corrected;
			best_k = k;
			best_ctz_group_size = 1;
		}
		vals_diff_sorted_window_ctz_std = array_std(vals_diff_sorted_window, nnz, vals_diff_sorted_window_ctz_avg, get_double_trailing_zeros);
		vals_diff_sorted_window_unique_num = array_unique_num(vals_diff_sorted_window, nnz);
		vals_diff_sorted_window_exp_unique_num = array_unique_num(vals_diff_sorted_window, nnz, get_double_exponent);
		array_min_max(windows_exp_unique_num, num_windows, &windows_exp_unique_num_min, NULL, &windows_exp_unique_num_max, NULL);
		windows_exp_unique_num_avg = array_mean(windows_exp_unique_num, num_windows);
		...
		fprintf(stderr, "vals_diff_sorted_window (2^%ld) ctz avg - %ld - 6 = %.3lf\n", k, k, ctz_corrected);
		fprintf(stderr, "vals_diff_sorted_window (2^%ld) ctz std = %.3lf\n", k, vals_diff_sorted_window_ctz_std);
		for (ctz_group_size=4;ctz_group_size<=16;ctz_group_size+=4)
		{
			vals_diff_sorted_window_grouped_ctz_avg = array_mean_of_windowed_ctz_min(vals_diff_sorted_window, nnz, ctz_group_size);
	...
	fprintf(stderr, "vals_diff_sorted_window best ctz corrected avg = %.3lf\n", ctz_corrected_max);
	...
	for (k=4;k<=14;k++)
	{
		...
		vals_cluster_centers_diffs_avg = array_mean(vals_cluster_centers_diffs, nnz);
		vals_cluster_centers_diffs_std = array_std(vals_cluster_centers_diffs, nnz, vals_cluster_centers_diffs_avg);
		vals_cluster_centers_diffs_ctz_avg = array_mean(vals_cluster_centers_diffs, nnz, get_double_trailing_zeros);
		...
		best_ctz_group_size = 1;
		vals_cluster_centers_diffs_ctz_std = array_std(vals_cluster_centers_diffs, nnz, vals_cluster_centers_diffs_ctz_avg, get_double_trailing_zeros);
...
csr_matrix_features(char * title_base, _TYPE_I * row_ptr, _TYPE_I * col_idx, long m, long n, long nnz, int do_plot)
{
	...
	nnz_per_row_std = array_std(degrees_rows, m, nnz_per_row_avg);

	array_min_max(degrees_cols, n, &nnz_per_col_min, NULL, &nnz_per_col_max, NULL);
	nnz_per_col_avg = ((double) nnz) / n;
	nnz_per_col_std = array_std(degrees_cols, n, nnz_per_col_avg);

	array_min_max(bandwidths, m, &bw_min, NULL, &bw_max, NULL);
	bw_avg = array_mean(bandwidths, m);
	bw_std = array_std(bandwidths, m, bw_avg);

	array_min_max(scatters, m, &sc_min, NULL, &sc_max, NULL);
	sc_avg = array_mean(scatters, m);
	sc_std = array_std(scatters, m, sc_avg);
	...
	num_neigh_avg = array_mean(num_neigh, nnz);
	num_neigh_std = array_std(num_neigh, nnz, num_neigh_avg);
	...
	nnz_col_dist_avg = array_mean(nnz_col_dist, nnz);
	nnz_col_dist_std = array_std(nnz_col_dist, nnz, nnz_col_dist_avg);

	array_min_max(group_col_dist, num_groups, &group_col_dist_min, NULL, &group_col_dist_max, NULL);
	group_col_dist_avg = array_mean(group_col_dist, num_groups);
	group_col_dist_std = array_std(group_col_dist, num_groups, group_col_dist_avg);

	array_min_max(group_sizes, num_groups, &group_sizes_min, NULL, &group_sizes_max, NULL);
	group_sizes_avg = array_mean(group_sizes, num_groups);
	group_sizes_std = array_std(group_sizes, num_groups, group_sizes_avg);

	array_min_max(groups_per_row, m, &groups_per_row_min, NULL, &groups_per_row_max, NULL);
	groups_per_row_avg = array_mean(groups_per_row, m);
	groups_per_row_std = array_std(groups_per_row, m, groups_per_row_avg);
