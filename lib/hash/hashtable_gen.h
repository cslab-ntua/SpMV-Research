	...
	#error "HASHTABLE_GEN_TYPE_1 not defined: [ Key Type ] Must be a basic type or array, NOT a structure (can't be safely compared without user intervention)."
	...
	#error "HASHTABLE_GEN_TYPE_3 not defined: [ Bucket Size Type (Signed Integer) ] Decide while considering the ratio of the hashtable size versus the total inserted data and also the total amount of inserted data (a small %% of a big number can also be a big number)"
 ...
 *     We need to locate the buckets with big traffic, i.e. values that appear multiple times (failed insertions).
...
#undef  HASHTABLE_GEN_MRU_s
#define HASHTABLE_GEN_MRU_s  HASHTABLE_GEN_EXPAND(HASHTABLE_GEN_MRU_s)
struct HASHTABLE_GEN_MRU_s {
	int n;
	int next;
...
/* 'counter':
 *     There is a common pitfall with 'mru_keys', which is the symmetric matrices.
 ...
 *     if we fail the insert at least some lower bound, keeping counts in 'counter'.
...
struct hashtable_bucket {
	...
	struct HASHTABLE_GEN_MRU_s * mru_keys;
};
...
int hashtable_insert(struct hashtable * ht, _TYPE_K key
	#if !HASHTABLE_GEN_VALUE_SAME_AS_KEY
		, _TYPE_V value
	#endif
	);
...
int hashtable_insert_concurrent(struct hashtable * ht, _TYPE_K key
	#if !HASHTABLE_GEN_VALUE_SAME_AS_KEY
		, _TYPE_V value
	#endif
	);
