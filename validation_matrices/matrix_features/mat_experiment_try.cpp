...
int main(int argc, char **argv)
{
	...
	int nnz_per_thread = 5;
	int threads_per_block = 128;
	int blocks_per_sm = 16;
	int sm_count = 132;

	int nnz_per_partition = sm_count * blocks_per_sm * threads_per_block * nnz_per_thread;
	printf("nnz_per_partition = %d\n", nnz_per_partition);

	int partitions = (nnz + nnz_per_partition - 1) / nnz_per_partition;
	printf("partitioning... in %d partitions\n", partitions);
	...
	_TYPE_I * partition_start_column = (typeof(partition_start_column)) malloc((partitions+1) * sizeof(*partition_start_column));
	_TYPE_I curr_partition_nnz = 0;
	int cnt_p = 0;
	partition_start_column[cnt_p++] = 0; // first partition begins from column 0

	for(_TYPE_I i=0; i<n; i++){
		_TYPE_I curr_col_nnz = col_ptr[i+1] - col_ptr[i];
		if(curr_partition_nnz + curr_col_nnz > nnz_per_partition){
			partition_start_column[cnt_p++] = i;
			curr_partition_nnz = 0;
		}
		curr_partition_nnz += curr_col_nnz;
	}
	partition_start_column[partitions] = n;

	// int cnt_p2=0;
	// for(int i=0; i<n; i++){
	// 	if(i == partition_start_column[cnt_p2]){
	// 		_TYPE_I curr_partition_nnz = col_ptr[partition_start_column[cnt_p2+1]] - col_ptr[partition_start_column[cnt_p2]];
	// 		_TYPE_I partition_col_offset = partition_start_column[cnt_p2];
	// 		_TYPE_I partition_nnz_offset = col_ptr[partition_col_offset];
	// 		printf("partition %d: [%d - %d] column = %d, col_ptr = %d (nnz = %d), offset for n is %d, offset for nnz is %d\n", 
	// 			      cnt_p2, partition_start_column[cnt_p2], partition_start_column[cnt_p2+1]-1, 
	// 			      i, col_ptr[i], curr_partition_nnz,
	// 			      partition_col_offset, partition_nnz_offset);
	// 		cnt_p2++;
	...
	for(int i=0; i<partitions; i++){
		_TYPE_I partition_col_offset = partition_start_column[i];
		_TYPE_I partition_nnz_offset = col_ptr[partition_col_offset];

		_TYPE_I curr_partition_n = partition_start_column[i+1]-1 - partition_start_column[i] + 1;
		_TYPE_I curr_partition_nnz = col_ptr[partition_start_column[i+1]] - col_ptr[partition_col_offset];

		// printf("\npartition %d: [%d - %d] column = %d, col_ptr = %d (nnz = %d), offset for n is %d, offset for nnz is %d\n", 
		// 	      i, partition_start_column[i], partition_start_column[i+1]-1,
		// 	      partition_col_offset, col_ptr[partition_col_offset], curr_partition_nnz,
		// 	      partition_col_offset, partition_nnz_offset);
		// printf("partition %d: columns = %d ([%d-%d]) nonzeros = %d, partition_col_offset = %d\n", i, curr_partition_n, partition_start_column[i], partition_start_column[i+1]-1, curr_partition_nnz, partition_col_offset);

		double time_all = time_it(1,
		// create a new csc representation for this partition
		_TYPE_I * col_ptr_p; //for CSC format
		_TYPE_I * row_idx_p;
		_TYPE_V * val_c_p;

		col_ptr_p = (typeof(col_ptr_p)) malloc((curr_partition_n+1) * sizeof(*col_ptr_p));
		row_idx_p = (typeof(row_idx_p)) malloc(curr_partition_nnz * sizeof(*row_idx_p));
		val_c_p = (typeof(val_c_p)) malloc(curr_partition_nnz * sizeof(*val_c_p));

		for(int j=0; j<curr_partition_n+1; j++)
			col_ptr_p[j] = col_ptr[j + partition_col_offset] - partition_nnz_offset;
		for(int j=0; j<curr_partition_nnz; j++){
			row_idx_p[j] = row_idx[j + partition_nnz_offset];
			val_c_p[j] = val_c[j + partition_nnz_offset];
		}

		_TYPE_I * col_ptr_ps; //for CSC format
		_TYPE_I * row_idx_ps;
		_TYPE_V * val_c_ps;

		col_ptr_ps = (typeof(col_ptr_ps)) malloc((curr_partition_n+1) * sizeof(*col_ptr_ps));
		row_idx_ps = (typeof(row_idx_ps)) malloc(curr_partition_nnz * sizeof(*row_idx_ps));
		val_c_ps = (typeof(val_c_ps)) malloc(curr_partition_nnz * sizeof(*val_c_ps));

		_TYPE_I * original_positions;
		original_positions = (_TYPE_I*) malloc(curr_partition_n * sizeof(_TYPE_I));
		csc_sort_by_col_size(curr_partition_n, row_idx_p, col_ptr_p, val_c_p, row_idx_ps, col_ptr_ps, val_c_ps, original_positions);
		free(original_positions);


		char * membership_suffix = (typeof(membership_suffix)) malloc(100*sizeof(*membership_suffix));
		sprintf(membership_suffix, "p%d_sorted", i);
		int plot = 0;
		int store = 0;
		int mode = 0;
		// feature_plot_store_csc_matrix(file_in, membership_suffix, plot, store, m, curr_partition_n, curr_partition_nnz, row_idx_p, col_ptr_p, val_c_p, mode);
		feature_plot_store_csc_matrix(file_in, membership_suffix, plot, store, m, curr_partition_n, curr_partition_nnz, row_idx_ps, col_ptr_ps, val_c_ps, mode);
		free(membership_suffix);
		...
		// printf("partition %d time: %.4lf ms\n", i, time_all*1000);
	}
