...
int main(int argc, char **argv)
{
	...
	if(!artificial) {
		...
		csr_kmeans_char_reorder_row_batch(row_ptr, col_idx, val, row_ptr_reorder_r, col_idx_reorder_r, val_reorder_r, &original_row_positions, m, n, nnz, batch, numClusters, threshold, loop_threshold, num_windows_row, seed, rc_r, rc_c, rc_v);
		);
		...
		sprintf(membership_suffix, "row_reordered_window_%d_batch_%d_clusters_%d_char", window_width, batch, numClusters);
		...
		membership_suffix = (typeof(membership_suffix)) malloc(100*sizeof(*membership_suffix));
		sprintf(membership_suffix, "row_col_reordered_window_%d_batch_%d_clusters_%d_char", window_width, batch, numClusters);
