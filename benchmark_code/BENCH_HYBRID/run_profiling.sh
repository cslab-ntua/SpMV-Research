...
matrices_weird=( 
    $(
        for ((i=0;i<${#matrices_weird[@]};i++)); do
            m="${matrices_weird[i]}.mtx"
            for d in "${validation_dirs[@]}"; do
                if [[ -f "${d}/${m}" ]]; then
                    echo "${d}/${m}"
                    break
    ...
    for ((i=0;i<${#array_cores[@]};i++)); do
        ...
        while :; do
            if [[ "$prog" == *"spmv_sell-C-s_d.exe"* ]]; then
                ...
                if [[ "$prog" == *"timed_v2"* && -f temp_tb.txt ]]; then
                    mv temp_tb.txt timer_block/logs/${mtx_name}_tb.txt
                fi
                if [[ "$prog" == *"timed_v3"* && -f temp_tb.txt ]]; then
                    mv temp_tb.txt timer_block/logs/${mtx_name}_tb.txt
                    mv temp_exclude_1sig.txt timer_block/logs/${mtx_name}_excl1sig.txt
                    mv temp_exclude_2sig.txt timer_block/logs/${mtx_name}_excl2sig.txt
                    mv normal_1.mtx ../../validation_matrices/download_matrices/new_folder/${mtx_name}_normal_1.mtx
                    mv outliers_1.mtx ../../validation_matrices/download_matrices/new_folder/${mtx_name}_outliers_1.mtx
                    mv normal_2.mtx ../../validation_matrices/download_matrices/new_folder/${mtx_name}_normal_2.mtx
                    mv outliers_2.mtx ../../validation_matrices/download_matrices/new_folder/${mtx_name}_outliers_2.mtx
                fi
                if [[ "$prog" == *"timed_v4"* && -f temp_tw.txt ]]; then
                    mv temp_tw.txt timer_block/warp_logs/${mtx_name}_tw.txt
                fi
                if [[ "$prog" == *"timed_v5"* && -f gpu_part.mtx && -f cpu_part.mtx ]]; then
                    mv gpu_part.mtx ../../validation_matrices/download_matrices/new_folder/${mtx_name}_gpu_part.mtx
                    mv cpu_part.mtx ../../validation_matrices/download_matrices/new_folder/${mtx_name}_cpu_part.mtx
                fi
                if [[ "$prog" == *"timed_v6"* && -f temp_tw.txt && -f temp_xcl.txt ]]; then
                    mv temp_tw.txt timer_block/warp_logs/${mtx_name}_tw.txt
                    mv temp_xcl.txt timer_block/warp_logs/${mtx_name}_xcl.txt
                fi

                # for ((i=1; i<=5; i++)); do
                #     "$prog" "${prog_args[@]}"  2>'tmp.err'
                #     mkdir -p timer_block/warp_logs/run${i}
                #     mv temp_tw.txt timer_block/warp_logs/run${i}/${mtx_name}_tw.txt
                # done
                ret="$?"
            fi
...
for format_name in "${!progs[@]}"; do
    ...
    # Save original stdout/stderr so we can restore between reps when output_to_files=1.
    exec 3>&1
    exec 4>&2
    ...
    for ((i=0;i<rep;i++)); do
        ...
        if ((output_to_files)); then
            ...
            # suffix=""
            suffix="_profiling"
            if ((rep > 1)); then suffix="_profiling_rep${i}"; fi
        ...
        # Restore stdout/stderr before the next rep (or before moving to next format).
        if ((output_to_files)); then
            exec 1>&3
            exec 2>&4
        fi
