...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * ia, INT_T * ja, ValueTypeReference * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), ia(ia), ja(ja), a(a)
	{
		...
		cuda_assert(cudaMalloc(&row_blocks_d, row_blocks_cnt * sizeof(*row_blocks_d)));
		...
		if(TIME_IT){
			...
			cuda_assert(cudaEventCreate(&startEvent_memcpy_row_blocks));
			cuda_assert(cudaEventCreate(&endEvent_memcpy_row_blocks));
		...
		cuda_assert(cudaMallocHost(&row_blocks_h, row_blocks_cnt * sizeof(*row_blocks_h)));
		...
		if(TIME_IT) cuda_assert(cudaEventRecord(startEvent_memcpy_row_blocks));
		cuda_assert(cudaMemcpyAsync(row_blocks_d, row_blocks_h, row_blocks_cnt * sizeof(*row_blocks_d), cudaMemcpyHostToDevice, stream));
		if(TIME_IT) cuda_assert(cudaEventRecord(endEvent_memcpy_row_blocks));
		...
		if(TIME_IT){
			...
			cuda_assert(cudaEventSynchronize(endEvent_memcpy_row_blocks));
			...
			cuda_assert(cudaEventElapsedTime(&memcpyTime_cuda_row_blocks, startEvent_memcpy_row_blocks, endEvent_memcpy_row_blocks));
	...
	~CSRArrays()
	{
		...
		cuda_assert(cudaFree(row_blocks_d));
		...
		cuda_assert(cudaFreeHost(row_blocks_h));
		...
		if(TIME_IT){
			...
			cuda_assert(cudaEventDestroy(startEvent_memcpy_row_blocks));
			cuda_assert(cudaEventDestroy(endEvent_memcpy_row_blocks));
		}
