...
struct COOArrays : Matrix_Format
{
	...
	COOArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		time = time_it(1,
			_Pragma("omp parallel")
			{
				...
				for (j=0;j<nnz;j++)
				{
					if (j % NNZ_PER_THREAD == 0)
					{
						ia_h[j] = 0;
						continue;
					}
					macros_binary_search(row_ptr, 0, m, j, &i, NULL);           // Index boundaries are inclusive.
					macros_binary_search(row_ptr, 0, m, j-1, &i_prev, NULL);           // Index boundaries are inclusive.
					ia_h[j] = i - i_prev;
				}
			}
		);
...
__device__
void
spmv_last_block(INT_T * thread_i_s, unsigned char * ia, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
...
__device__
void
spmv_full_warp(group_t g, int single_row, int i_s, int j_s, int j_b_s, int j_w_s, unsigned char * ia, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
	extern __shared__ char sm[];
	const int tidl = g.thread_rank();   // Group lane.
	int i, j, jj;
...
__device__
void
spmv_full_block(INT_T * thread_i_s, unsigned char * ia, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	// extern __shared__ char sm[];
	// const int tidb = threadIdx.x;
	const int tid = cuda_get_thread_num_bc();
	...
	i_s = thread_i_s[tid] & 0x7FFFFFFF;
	int single_row = thread_i_s[tid] & 0x80000000;
	...
	// int single_row = 0;
	thread_block_tile<32> tile32 = tiled_partition<32>(this_thread_block());
	spmv_full_warp(tile32, single_row, i_s, j_s, j_b_s, j_w_s, ia, ja, a, x, y);
}


__global__
void
gpu_kernel_spmv_row_indices_continuous(INT_T * thread_i_s, unsigned char * ia, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	int grid_size = gridDim.x;
	int block_id = blockIdx.x;
	if (block_id == grid_size - 1)
		spmv_last_block(thread_i_s, ia, ja, a, m, n, nnz, x, y);
	else
		spmv_full_block(thread_i_s, ia, ja, a, m, n, nnz, x, y);
}
...
compute_coo(COOArrays * restrict coo, ValueType * restrict x, ValueType * restrict y)
{
	...
	gpu_kernel_spmv_row_indices_continuous<<<grid_dims, block_dims, shared_mem_size>>>(coo->thread_i_s_d, coo->ia_d, coo->ja_d, coo->a_d, coo->m, coo->n, coo->nnz, coo->x_d, coo->y_d);
