...
extern "C"{
	...
	#define FUNCTOOLS_GEN_SUFFIX  _CUDA_LUT_CU
	...
	#define BUCKETSORT_GEN_SUFFIX  _CUDA_LUT_CU
	...
	#define SAMPLESORT_GEN_SUFFIX  _CUDA_LUT_CU
...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueTypeReference * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		a_idx = (typeof(a_idx)) malloc(a_idx_len * nnz_extended * sizeof(*a_idx));
		_Pragma("omp parallel")
		{
			long j, k;
			union {
		...
		cuda_assert(cudaMalloc(&a_idx_d, a_idx_len * nnz_extended * sizeof(*a_idx_d)));
		...
		cuda_assert(cudaMemcpy(a_idx_d, a_idx, a_idx_len * nnz_extended * sizeof(*a_idx_d), cudaMemcpyHostToDevice));
	}
...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueTypeReference * values, long m, long n, long nnz, long symmetric, long symmetry_expanded)
{
	...
	// for (long i=0;i<10;i++)
		// printf("%d\n", row_ptr[i]);
	csr->mem_footprint = csr->nnz_extended * csr->a_idx_len + csr->num_unique * sizeof(ValueType) + csr->nnz_extended * sizeof(INT_T) + (m+1) * sizeof(INT_T);
	char *format_name;
	format_name = (char *)malloc(100*sizeof(char));
	snprintf(format_name, 100, "Custom_CSR_CUDA_lut_b%d_nnz%ld", BLOCK_SIZE, csr->nnz_per_thread);
...
template <const int nnz_per_thread, typename LutIdxType>
__device__
void
spmv_full_block(INT_T * thread_i_s, INT_T * thread_warp_i_s, INT_T * thread_warp_i_e, INT_T * row_ptr, INT_T * ja, ValueTypeStored * lut, LutIdxType * a_idx, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	...
	{
		sum = __fma_rn((ValueType) lut[a_idx[jj]], x[ja[jj] & 0x7FFFFFFF], sum);
		row |= ja[jj];
	}
...
__global__
void
gpu_kernel_spmv_row_indices_continuous(INT_T * thread_i_s, INT_T * thread_warp_i_s, INT_T * thread_warp_i_e, INT_T * row_ptr, INT_T * ja, ValueTypeStored * lut, LutIdxType * a_idx, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	spmv_full_block<nnz_per_thread, LutIdxType>(thread_i_s, thread_warp_i_s, thread_warp_i_e, row_ptr, ja, lut, a_idx, m, n, nnz, x, y);
}
...
compute_csr(CSRArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	case 1:
		switch (csr->a_idx_len) {
		case 1:
			gpu_kernel_spmv_row_indices_continuous<1, uint8_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint8_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		case 2:
			gpu_kernel_spmv_row_indices_continuous<1, uint16_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint16_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		default:
			gpu_kernel_spmv_row_indices_continuous<1, uint32_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint32_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
		}
		break;
	case 2:
		switch (csr->a_idx_len) {
		case 1:
			gpu_kernel_spmv_row_indices_continuous<2, uint8_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint8_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		case 2:
			gpu_kernel_spmv_row_indices_continuous<2, uint16_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint16_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		default:
			gpu_kernel_spmv_row_indices_continuous<2, uint32_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint32_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
		}
		break;
	case 3:
		switch (csr->a_idx_len) {
		case 1:
			gpu_kernel_spmv_row_indices_continuous<3, uint8_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint8_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		case 2:
			gpu_kernel_spmv_row_indices_continuous<3, uint16_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint16_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		default:
			gpu_kernel_spmv_row_indices_continuous<3, uint32_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint32_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
		}
		break;
	case 4:
		switch (csr->a_idx_len) {
		case 1:
			gpu_kernel_spmv_row_indices_continuous<4, uint8_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint8_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		case 2:
			gpu_kernel_spmv_row_indices_continuous<4, uint16_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint16_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		default:
			gpu_kernel_spmv_row_indices_continuous<4, uint32_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint32_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
		}
		break;
	default:
		switch (csr->a_idx_len) {
		case 1:
			gpu_kernel_spmv_row_indices_continuous<5, uint8_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint8_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		case 2:
			gpu_kernel_spmv_row_indices_continuous<5, uint16_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint16_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
			break;
		default:
			gpu_kernel_spmv_row_indices_continuous<5, uint32_t><<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->lut_d, (uint32_t *) csr->a_idx_d, csr->m, csr->n, csr->nnz_extended, csr->x_d, csr->y_d);
		}
	}
