...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		gpuCudaErrorCheck(cudaMalloc(&lut_d, num_unique * sizeof(*lut_d)));
		gpuCudaErrorCheck(cudaMalloc(&a_idx_d, a_idx_len * nnz_expanded * sizeof(*a_idx_d)));
		...
		gpuCudaErrorCheck(cudaMemcpy(lut_d, lut, num_unique * sizeof(*lut_d), cudaMemcpyHostToDevice));
		gpuCudaErrorCheck(cudaMemcpy(a_idx_d, a_idx, a_idx_len * nnz_expanded * sizeof(*a_idx_d), cudaMemcpyHostToDevice));
	}
