...
extern "C"{
	...
	#define FUNCTOOLS_GEN_SUFFIX  _CUDA_CSR_TRANSPOSE_N_THREADS_PER_ROW_CU
...
transpose_split_rows(T * A, INT_T m, INT_T n, INT_T num_row_segments)
{
	...
	INT_T num_row_segments_expanded = n % num_row_segments;  // The initial segments that expand to n_div+1 due to n%num_row_segments != 0;
	...
	for (i=0;i<m;i++)
	{
		...
		for (ii=0;ii<num_row_segments;ii++)
		{
			...
			INT_T n_actual = ii < num_row_segments_expanded ? n_div + 1 : n_div;
			INT_T i_r = ii < num_row_segments_expanded ? ii : num_row_segments_expanded;
			for (j_new=0;j_new<n_actual;j_new++)
			{
				INT_T j_r = j_new < n_div ? i_new : i * num_row_segments_expanded + ii;
...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueTypeReference * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		if (nnz_per_row < 1)
			nnz_per_row = 1;
		// NNZ_PER_THREAD = nnz_per_row / THREADS_PER_ROW;

		nnz_per_block = BLOCK_SIZE * nnz_per_row / THREADS_PER_ROW;
		nnz_per_warp = 32 * nnz_per_row / THREADS_PER_ROW;
		// long i, j;
		...
		num_threads = THREADS_PER_ROW * m;
		num_active_threads = THREADS_PER_ROW * m;
		...
		_Pragma("omp parallel")
		{
			long j;
			_Pragma("omp for")
			for (j=0;j<(num_active_thread_warps-1)*nnz_per_warp;j+=nnz_per_warp)
			{
				long j_e = j + nnz_per_warp;
				if (j_e < nnz_expanded)
				{
					transpose_split_rows(&a_h[j], 16, nnz_per_row, THREADS_PER_ROW);
					transpose_split_rows(&ja_h[j], 16, nnz_per_row, THREADS_PER_ROW);
				}
			}
		}
		...
		// {
			...
			// transpose_split_rows(&a_h[j], last_warp_num_active_threads, NNZ_PER_THREAD);
			// transpose_split_rows(&ja_h[j], last_warp_num_active_threads, NNZ_PER_THREAD);
		// }
		...
		/* _Pragma("omp parallel")
		{
			...
			for (j=0;j<(num_thread_blocks-1)*nnz_per_block;j+=BLOCK_SIZE*NNZ_PER_THREAD)
			{
				...
				if (j_e < nnz_expanded)
				{
					transpose_split_rows(&a_h[j], BLOCK_SIZE, NNZ_PER_THREAD);
					transpose_split_rows(&ja_h[j], BLOCK_SIZE, NNZ_PER_THREAD);
				}
			}
		}
		_Pragma("omp parallel")
		{
			...
			for (j=(num_thread_blocks-1)*nnz_per_block;j<num_active_thread_warps*nnz_per_warp;j+=32*NNZ_PER_THREAD)
			{
				...
				if (j_e < nnz_expanded)
				{
					transpose_split_rows(&a_h[j], 32, NNZ_PER_THREAD);
					transpose_split_rows(&ja_h[j], 32, NNZ_PER_THREAD);
				}
			}
		}
		// int last_warp_num_active_threads = num_active_threads % 32;
...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueTypeReference * values, long m, long n, long nnz, long symmetric, long symmetry_expanded)
{
	...
	snprintf(format_name, 100, "Custom_CSR_CUDA_one_row_per_thread_b%d", BLOCK_SIZE);
...
spmv_last_block(group_t g, INT_T * row_ptr, INT_T * ja, ValueType * a, int m, int n, int nnz, ValueType * restrict x, ValueType * restrict y, int num_active_threads)
{
	...
	i = tid / THREADS_PER_ROW;
	if (wid < num_active_thread_warps - 1)
	{
		j_w_s = row_ptr[wid * g.size() / THREADS_PER_ROW];
		j_w_e = row_ptr[(wid+1) * g.size() / THREADS_PER_ROW];
		// j_s = row_ptr[i];
		// j_e = row_ptr[i+1];
