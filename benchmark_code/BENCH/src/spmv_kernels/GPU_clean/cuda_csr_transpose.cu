...
__device__
void
spmv_full_warp(group_t g, int single_row, int i_s, int j_s, int j_b_s, int j_w_s, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
...
spmv_full_block(INT_T * thread_i_s, INT_T * thread_warp_i_s, INT_T * thread_warp_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	...
	// single_row = thread_i_s[tid] & 0x80000000;
	...
	// if (j_s >= row_ptr[i_s+1])
		// printf("test");

	// single_row = ja[j_w_s] & 0x80000000;
	single_row = ja[j_s] & 0x80000000 ? -1 : 0;
	// single_row = 0;
	thread_block_tile<32> tile32 = tiled_partition<32>(this_thread_block());
