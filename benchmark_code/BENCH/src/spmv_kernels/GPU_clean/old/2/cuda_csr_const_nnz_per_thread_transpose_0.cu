		...
		time_balance = time_it(1,
			_Pragma("omp parallel")
			{
				...
				for (i=0;i<num_thread_warps;i++)
				{
					...
					if (thread_warp_j_s > nnz)
						thread_warp_j_s = nnz;
					macros_binary_search(row_ptr, 0, m, thread_warp_j_s, &lower_boundary, NULL);           // Index boundaries are inclusive.
					thread_warp_i_s[i] = lower_boundary;
					thread_warp_j_e = thread_warp_j_s + nnz_per_warp;
					if (thread_warp_j_e > nnz)
						thread_warp_j_e = nnz;
					macros_binary_search(row_ptr, 0, m, thread_warp_j_e, NULL, &higher_boundary);           // Index boundaries are inclusive.
...
__device__ void spmv_last_block(INT_T * thread_i_s, INT_T * thread_warp_i_s, INT_T * thread_warp_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	...
	// i = thread_i_s[tid] & 0x7FFFFFFF;
	i_s = thread_warp_i_s[wid] & 0x7FFFFFFF;
...
__device__ void spmv_full_block(INT_T * thread_i_s, INT_T * thread_warp_i_s, INT_T * thread_warp_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
	// extern __shared__ char sm[];
	const int tid = cuda_get_thread_num_bc();
	...
	int single_row = thread_warp_i_s[wid] & 0x80000000;
	i_s = thread_warp_i_s[wid] & 0x7FFFFFFF;
	// i_s = thread_warp_i_s[wid];
	i_e = thread_warp_i_e[wid];
...
__global__ void gpu_kernel_spmv_row_indices_continuous(INT_T * thread_i_s, INT_T * thread_warp_i_s, INT_T * thread_warp_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y)
{
...
compute_csr(CSRArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	gpu_kernel_spmv_row_indices_continuous<<<grid_dims, block_dims, shared_mem_size>>>(csr->thread_i_s_d, csr->thread_warp_i_s_d, csr->thread_warp_i_e_d, csr->row_ptr_d, csr->ja_d, csr->a_d, csr->m, csr->n, csr->nnz, csr->x_d, csr->y_d);
