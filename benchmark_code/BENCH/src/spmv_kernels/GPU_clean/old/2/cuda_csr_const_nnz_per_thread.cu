...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		time_balance = time_it(1,
			...
			{
				INT_T thread_block_j_s, thread_block_j_e;
				...
				for (i=0;i<num_thread_blocks;i++)
				{
					...
					thread_block_j_e = thread_block_j_s + nnz_per_block;
					if (thread_block_j_e > nnz)
						thread_block_j_e = nnz;
					macros_binary_search(row_ptr, 0, m, thread_block_j_e, NULL, &higher_boundary);           // Index boundaries are inclusive.
					thread_block_i_e[i] = higher_boundary;
				}
			}
		);
