...
extern "C"{
	...
	#define QUICKSORT_GEN_SUFFIX  _CUDA_SELL_SORTED_HYBRID_simple
	#include "sort/quicksort/quicksort_gen.c"
	static inline
	int
	quicksort_cmp(INT_T a, INT_T b, __attribute__((unused)) void * aux_data)
	{
...
void
csr_row_segment_pattern_reorder(INT_T * row_ptr, INT_T * col_idx, long m, long n, long nnz, long crossover_row,
		INT_T ** permutation_ret)
{
	int num_threads = omp_get_max_threads();
	INT_T * reverse_permutation=NULL, * permutation=NULL;
	long num_bitmaps = 30;
	...
	if (permutation_ret == NULL)
		error("permutation_ret is NULL");

	int32_t bitmap;

	reverse_permutation = (typeof(reverse_permutation)) malloc(m * sizeof(*reverse_permutation));
	permutation = (typeof(permutation)) malloc(m * sizeof(*permutation));
	...
	_Pragma("omp parallel")
	{
		...
		uint16_t hash_all, hash_no_outliers;
		...
		for (i=i_s;i<i_e;i++)
		{
			...
			{
				col_idx_buf[jj] = col_idx[j];
			}

			// sort col_idx_buf
			quicksort_CUDA_SELL_SORTED_HYBRID_simple(col_idx_buf, degree, NULL, partitions_buf);

			hash_all = (typeof(hash_all)) xorshift64(col_idx_buf, degree * sizeof(*col_idx_buf), 0);
			...
			for (k=0;k<num_bitmaps;k++)
			{
				...
				// threshold = degree / (8*num_segments);
				if (threshold == 0)
					threshold = 1;
				...
				for (jj=0;jj<degree;jj++)
				{
					...
					if (segment >= num_segments)
						error("segment=%ld >= num_segments=%ld", segment, num_segments);
					counts[segment]++;
				}
			...
			if (degree > 0)
			{
				hash_no_outliers = (typeof(hash_no_outliers)) xorshift64(col_idx_buf, degree * sizeof(*col_idx_buf), 0);
				bitstream_write(&bs, hash_no_outliers, 8 * sizeof(hash_no_outliers));
				bitstream_write(&bs, hash_all, 8 * sizeof(hash_all));
			}
	...
	samplesort(&reverse_permutation[crossover_row], m-crossover_row, NULL);
	...
	_Pragma("omp parallel")
	{
		...
		for (i=0;i<m;i++)
		{
			permutation[reverse_permutation[i]] = i;
		}
...
struct SELLArrays : Matrix_Format
{
	...
	SELLArrays(INT_T * row_ptr, INT_T * ja, ValueTypeReference * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		double time_sort_rows = time_it(1,
			...
			/* Sort by row size. */
			csr_row_segment_pattern_reorder(row_ptr, ja, m, n, nnz, crossover_row, &row_permutation);
