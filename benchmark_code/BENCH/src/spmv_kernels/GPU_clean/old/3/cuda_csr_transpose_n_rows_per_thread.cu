...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz)
	{
		...
		_Pragma("omp parallel")
		{
			...
			for (j=0;j<num_active_thread_warps*nnz_per_warp;j+=32*NNZ_PER_THREAD)
			{
...
__device__
void
spmv_last_warps(group_t g, int m, int i_s, int j_w_s, int j_b_s, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
	// extern __shared__ double x_smem[];
	const int tidw = g.thread_rank();   // Group lane.
	int i, i_e, j, jj, j_s, j_e;
	double sum = 0;
	double x_buf;
	...
	for (i=i_s;i<i_e && i<m;i++)
	{
		...
		// for (jj=j_w_s+tidw;jj<j_w_s+tidw+32*165;jj+=g.size())
...
__device__
void
spmv_full_warp_last_block(group_t g, int i_s, int j_w_s, int j_b_s, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
	// extern __shared__ double x_smem[];
	const int tidw = g.thread_rank();   // Group lane.
	int i, i_e, j, jj, j_s, j_e;
...
__device__
void
spmv_last_block(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y, int num_active_threads)
{
	...
	__attribute__((unused)) int i_s, i_e, j_s, j_w_s, j_b_s, k;
	j_w_s = row_ptr[ROWS_PER_THREAD * wid * 32];
	// j_b_s = row_ptr[ROWS_PER_THREAD * block_id * BLOCK_SIZE];
	i_s = ROWS_PER_THREAD * tid;
	thread_block_tile<32> tile32 = tiled_partition<32>(this_thread_block());
	if (wid < num_active_thread_warps - 1)
		spmv_full_warp_last_block(tile32, i_s, j_w_s, j_b_s, row_ptr, ja, a, x, y);
	else
	{
		spmv_last_warps(tile32, m, i_s, j_w_s, j_b_s, row_ptr, ja, a, x, y);
	}
}
...
__device__
void
spmv_full_warp(group_t g, int i_s, int j_w_s, int j_b_s, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
	// extern __shared__ double x_smem[];
	const int tidw = g.thread_rank();   // Group lane.
	int i, i_e, j, jj, j_s, j_e;
	...
	for (i=i_s;i<i_e;i++)
	{
		...
		// for (j=j_s,jj=j;j<j_e;j++,jj++)
		for (j=j_s;j<j_e;j++,jj+=g.size())
		// for (j=j_s;j<j_e;j++,jj+=BLOCK_SIZE)
		{
			sum = __fma_rn(a[jj], x[ja[jj]], sum);
		}
		y[i] = sum;
...
__device__
void
spmv_full_block(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y, int num_active_threads)
{
	// extern __shared__ char sm[];
	const int tid = cuda_get_thread_num_bc();
	// const int tid = blockIdx.x * BLOCK_SIZE + threadIdx.x;
	...
	// if (block_id != grid_size - 1)
		spmv_full_warp(tile32, i_s, j_w_s, j_b_s, row_ptr, ja, a, x, y);
	// else
	// {
		// int last_warp_num_active_threads = num_active_threads & 31;
		// if (last_warp_num_active_threads == 0)
			// last_warp_num_active_threads = 32;
		// spmv_last_warps(tile32, m, last_warp_num_active_threads, i_s, j_w_s, j_b_s, row_ptr, ja, a, x, y);
	// }
}
...
__global__
void
gpu_kernel_spmv_row_indices_continuous(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz, ValueType * restrict x, ValueType * restrict y, int num_active_threads)
{
	int grid_size = gridDim.x;
	int block_id = blockIdx.x;
	if (block_id != grid_size - 1)
		spmv_full_block(row_ptr, ja, a, m, n, nnz, x, y, num_active_threads);
	else
		spmv_last_block(row_ptr, ja, a, m, n, nnz, x, y, num_active_threads);
}
