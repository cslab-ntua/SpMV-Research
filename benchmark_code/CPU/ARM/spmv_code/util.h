	...
	while (true)
	{
		...
		else if (current_nz == -1)
		{
			...
			if ((!array) && (nparsed == 3))
			{
				...
				coo->rowind = (MKL_INT *) malloc(nr_nnzs * sizeof(MKL_INT));
				coo->colind = (MKL_INT *) malloc(nr_nnzs * sizeof(MKL_INT));
...
void CSR_to_BCSR(CSRArrays * csr, BCSRArrays* bcsr, const int blockSize)
{
	...
	bcsr->a = (ValueType *) malloc(bcsr->nbBlocks*bcsr->lb*bcsr->lb * sizeof(ValueType));
	bcsr->ia = (MKL_INT *) malloc((bcsr->nbBlockRows+1) * sizeof(MKL_INT));
	bcsr->ja = (MKL_INT *) malloc(bcsr->nbBlocks * sizeof(MKL_INT));
...
void CSR_to_DIA(CSRArrays * csr, DIAArrays* dia)
{
	...
	dia->val = (ValueType *) malloc(dia->ndiag*dia->lval * sizeof(ValueType));
	dia->idiag = (MKL_INT *) malloc(dia->ndiag * sizeof(MKL_INT));
...
void CSR_to_ELL(CSRArrays * csr, ELLArrays * ell)
{
	...
	ell->a = (ValueType *) malloc(ell->m * ell->width * sizeof(ValueType));
	ell->ja = (MKL_INT *) malloc(ell->m * ell->width * sizeof(MKL_INT));
	#pragma omp parallel
	{
		long i, j, k;
		#pragma omp for
