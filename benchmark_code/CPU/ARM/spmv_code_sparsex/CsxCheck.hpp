...
template<typename IndexType, typename ValueType>
void CheckResultArtificial(vector_t *result, ValueType alpha, vector_t *x, IndexType *row_ptr, IndexType *col_ind, ValueType *values, IndexType nr_rows, IndexType nr_cols)
{
  CSR<IndexType, ValueType> *csr = new CSR<IndexType, ValueType>(row_ptr, col_ind, values, nr_rows, nr_cols, 1);
...
void check_result_artificial(vector_t *result, double alpha, vector_t *x, 
                  int *row_ptr, int *col_ind, double *values, int nr_rows, int nr_cols);
SPX_END_C_DECLS__
