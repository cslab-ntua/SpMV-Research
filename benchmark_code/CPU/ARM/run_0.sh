...
bench()
{
    ...
    for group_size in $group_sizes; do                      
        ...
        do
            ...
            # "${interleaved[@]}" "$prog" "${prog_args[@]}" 2>>arm_compiler__arm_validation_matrices_t${t}_d_${iter}.csv
            # "${interleaved[@]}" "$prog" "${prog_args[@]}" 2>>arm_compiler_unopt__arm_validation_matrices_t${t}_d_${iter}.csv
            "${interleaved[@]}" "$prog" "${prog_args[@]}" 2>>arm_compiler_nonative__arm_validation_matrices_t${t}_d_${iter}.csv
