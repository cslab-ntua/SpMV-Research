...
static inline
long
compress_kernel_div(INT_T * row_ptr, INT_T * ja, ValueTypeReference * vals, long i_s, long i_t_s, long i_t_e, long j_s, unsigned char * buf, long num_vals_suggested, long * num_vals_out, [[gnu::unused]] long symmetric)
{
	int tnum = omp_get_thread_num();
	struct thread_data_kernel * tdk = tdks[tnum];
	...
	{
		error("empty packet, num_vals_suggested=%ld, new_num_vals=%ld", num_vals_suggested, new_num_vals);
