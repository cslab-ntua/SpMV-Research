...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	uint64_t row_bits = 0, col_bits = 0, col_bytes = 0;
	long num_rfs;
	...
	unsigned char * data_rows = &((unsigned char *) data_num_unique_vals_per_rf)[data_num_unique_vals_per_rf_bytes];
	const uint64_t data_rows_bytes = num_vals;

	unsigned char * data_cols = &data_rows[data_rows_bytes];
	const uint64_t data_cols_bytes = col_bytes * num_vals;

	unsigned char * data_val_lens = &data_cols[data_cols_bytes];
	...
	return data_intro_bytes + data_rfs_bytes + data_num_unique_vals_per_rf_bytes + data_rows_bytes + data_cols_bytes + data_val_lens_bytes + data_val_lanes_bytes;
}
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate,
		void (* gather_coords)(long i, unsigned char * data_cols, const uint64_t col_bytes, uint64_t col_bits, uint64_t * col_rel_out),
		void (* gather_coords_v)(long i, unsigned char * data_cols, const uint64_t col_bytes, uint64_t col_bits, vec_t(i64, VEC_LEN) * col_rel_out)
		)
{
	int tnum = omp_get_thread_num();
	struct packet_header * header;
	...
	const uint64_t col_bytes = (col_bits + 7) >> 3;
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=VEC_LEN)
		{
			...
			{
				k = i*rf + VEC_LEN*j;
				row_rel = vec_set_iter(i64, VEC_LEN, iter, *((uint8_t *) &data_rows[k+iter]));
				gather_coords_v(k, data_cols, col_bytes, col_bits, &col_rel);
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			for (j=rf_multiple;j<rf;j++)
			{
				...
				k = i*rf + j;
				row_rel = *((uint8_t *) &data_rows[k]);
				gather_coords(k, data_cols, col_bytes, col_bits, &col_rel);
				if (validate)
				{
		...
		data_rows += rf * num_rf_vals_unique;
		data_cols += col_bytes * rf * num_rf_vals_unique;
