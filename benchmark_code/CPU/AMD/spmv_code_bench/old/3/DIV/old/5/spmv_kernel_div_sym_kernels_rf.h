...
quicksort_cmp(int a, int b, struct cmp_data * aux)
{
	...
	int ra_mul = ra / 8, rb_mul = rb / 8;
	int ca_mul = ca / 256, cb_mul = cb / 256;
	double va=vals[a], vb=vals[b];
	int ret = 0;
...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	compare_data.cols = (unsigned int *) &ja[j_s];
...
void
mult_add_serial(ValueType * x, ValueType * y, ValueType * y_upper, vec_d_t val, vec_i_t row, vec_i_t col, long i_s, long i_e)
{
	union {
		ValueType f;
		#if DOUBLE == 0
	...
	for (long iter=0;iter<vec_len_pd;iter++)
	{
		...
		if (r != c)
		{
			...
			else
			{
				while (1)
				{
					y_buf.u = __atomic_load_n((ValueType_cast_int *) &y[c], __ATOMIC_RELAXED);
					sum.f = y_buf.f + prod;
					if (__atomic_compare_exchange_n((ValueType_cast_int *) &y[c], &y_buf.u, sum.u, 0, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
						break;
				}
				// #pragma omp atomic
				// y[c] += prod;
...
static __attribute__((always_inline)) inline
long
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, ValueType * restrict y_upper, long * restrict num_vals_out, const int validate, long i_s, long i_e,
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=vec_len_pd)
		{
			...
			for (j=0;j<rf;j++)
			{
				...
				{
					mult_add_serial(x, y, y_upper, val.d, row, col, i_s, i_e);
				}
			}
		}
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			for (j=0;j<rf_multiple;j+=vec_len_pd)
			{
				...
				{
					mult_add_serial(x, y, y_upper, val_buf, row, col, i_s, i_e);
				}
			}
...
static __attribute__((always_inline)) inline
long
decompress_and_compute_kernel_sort_diff_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, ValueType * restrict y_upper, long * restrict num_vals_out, const int validate, long i_s, long i_e)
{
	uint64_t row_bits, col_bits;
	unsigned char * data_intro = buf;
	uint64_t data_intro_bytes = 0;
	...
	// if (row_bits == 8)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c1, gather_coords_sparse_r1_c1_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c2, gather_coords_sparse_r1_c2_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c3, gather_coords_sparse_r1_c3_v);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c4, gather_coords_sparse_r1_c4_v);
		}
	}
	...
	if (row_bits == 0)
	{
		// return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0, gather_coords_sparse_r0_v, 0);
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c1, gather_coords_sparse_r0_c1_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c2, gather_coords_sparse_r0_c2_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c3, gather_coords_sparse_r0_c3_v);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c4, gather_coords_sparse_r0_c4_v);
		}
	}

	switch (coords_bytes) {
		case 1:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_dense_1, gather_coords_dense_1_v);
		case 2:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_dense_2, gather_coords_dense_2_v);
		case 3:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_dense_3, gather_coords_dense_3_v);
		case 4:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_dense_4, gather_coords_dense_4_v);
	}

	return decompress_and_compute_kernel_sort_diff_base(buf, x, y, y_upper, num_vals_out, validate, i_s, i_e, gather_coords_dense, gather_coords_dense_v);
...
static inline
long
decompress_and_compute_kernel_sort_diff(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, ValueType * restrict y_upper, long i_s, long i_e)
{
	return decompress_and_compute_kernel_sort_diff_select(buf, x, y, y_upper, NULL, 0, i_s, i_e);
}


static
long
decompress_kernel_sort_diff(ValueType * vals, unsigned char * restrict buf, long i_s, long i_e)
{
	long num_vals;
	int tnum = omp_get_thread_num();
	double * window = t_window[tnum];
	int * rev_permutation = t_rev_permutation[tnum];
	long i, bytes;
	bytes = decompress_and_compute_kernel_sort_diff_select(buf, NULL, NULL, NULL, &num_vals, 1, i_s, i_e);
