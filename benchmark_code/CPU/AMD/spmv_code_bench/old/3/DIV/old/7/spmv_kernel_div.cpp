	...
	CSRCVSArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		time_compress = time_it(1,
			...
			_Pragma("omp parallel")
			{
				...
				if (1)
				{
					double time_01, time_11;
					num_vals_max = (j + num_packet_vals <= j_e) ? num_packet_vals : j_e - j;
					decompress_kernel_sort_diff(&coo_ia_new[pos_decompress], &coo_ja_new[pos_decompress], &coo_a_new[pos_decompress], &num_vals_dc, &data[pos], i_s, i_e);
					...
					compress_kernel_sort_diff(row_ptr, ja, &a[j], i, i_s, i_e, j, &data[pos], num_vals_max, &num_vals);
					time_01 = time_it(50,
						decompress_kernel_sort_diff(&coo_ia_new[pos_decompress], &coo_ja_new[pos_decompress], &coo_a_new[pos_decompress], &num_vals_dc, &data[pos], i_s, i_e);
					);
					...
					printf("%ld: time_01 - time_11=%g\n", tnum, time_01 - time_11);
