...
#elif defined(DIV_TYPE_RF)
	#include "spmv_kernel_div_sym_kernels_rf.h"
#else
	#include "spmv_kernel_div_kernels.h"
#endif
...
struct CSRCVSArrays : Matrix_Format
{
	...
	CSRCVSArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		y_upper = (typeof(y_upper)) aligned_alloc(64, m * sizeof(*y_upper));
...
{
	if (!symmetric)
		error("only symmetric matrices are supported by this format");
	struct CSRCVSArrays * csr = new CSRCVSArrays(row_ptr, col_ind, values, m, n, nnz);
	#if defined(DIV_TYPE_SELECT)
		csr->format_name = (char *) "CSR_SYM_CV_STREAM_SELECT";
	#elif defined(DIV_TYPE_ORD2)                
		csr->format_name = (char *) "CSR_SYM_CV_STREAM_RF_ORD2";
	#elif defined(DIV_TYPE_COLS_SORT)           
		csr->format_name = (char *) "CSR_SYM_CV_STREAM_COLS_SORT";
	#elif defined(DIV_TYPE_RF)                  
		csr->format_name = (char *) "CSR_SYM_CV_STREAM_RF";
	#else                                       
		csr->format_name = (char *) "CSR_SYM_CV_STREAM";
	#endif
	return csr;
}
...
compute_csr_cv_stream(CSRCVSArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	#pragma omp parallel
	{
		...
		thread_time_compute[tnum] += time_it(1,
			...
			{
				pos += decompress_and_compute_kernel_sort_diff(&(data[pos]), x, y, csr->y_upper, i_s, i_e);
			}
		);
		#pragma omp barrier
		#pragma omp for
