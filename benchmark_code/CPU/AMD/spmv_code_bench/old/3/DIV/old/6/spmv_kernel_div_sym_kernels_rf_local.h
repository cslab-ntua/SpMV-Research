...
static inline
long
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, long i_t_s, long i_t_e, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	int tnum = omp_get_thread_num();
	...
	for (l=0,i=row_min,j=j_s; l<num_vals; l++,j++)
	{
		...
		if (tolerance > 0)
			window_buf[k] = reduce_precision(vals[l], tolerance);
		else
			window_buf[k] = vals[l];
		rows[k] = i;
		cols_buf[k] = ja[j];
...
void
mult_add_serial(ValueType * x, ValueType * y, vec_d_t val, vec_i_t row, vec_i_t col, [[gnu::unused]] long i_s, long i_e)
{
	ValueType v;
	long r, c;
	for (long iter=0;iter<vec_len_pd;iter++)
...
static __attribute__((always_inline)) inline
long
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate, long i_s, long i_e,
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=vec_len_pd)
		{
			...
			for (j=0;j<rf;j++)
			{
				...
				{
					mult_add_serial(x, y, val.d, row, col, i_s, i_e);
				}
			}
		}
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			for (j=0;j<rf_multiple;j+=vec_len_pd)
			{
				...
				{
					mult_add_serial(x, y, val_buf, row, col, i_s, i_e);
				}
			}
			...
			for (j=rf_multiple;j<rf;j++)
			{
				...
				else
				{
					...
					{
						if (col >= i_s && col < i_e)
						// if (col >= i_s && col < i_e && (labs(col - row) < 256))
							y[col] += v * x[row];
					}
				}
			}
...
static __attribute__((always_inline)) inline
long
decompress_and_compute_kernel_sort_diff_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate, long i_s, long i_e)
{
	uint64_t row_bits, col_bits;
	unsigned char * data_intro = buf;
	uint64_t data_intro_bytes = 0;
	...
	// if (row_bits == 8)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c1, gather_coords_sparse_r1_c1_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c2, gather_coords_sparse_r1_c2_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c3, gather_coords_sparse_r1_c3_v);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r1_c4, gather_coords_sparse_r1_c4_v);
		}
	}
	...
	if (row_bits == 0)
	{
		// return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0, gather_coords_sparse_r0_v, 0);
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c1, gather_coords_sparse_r0_c1_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c2, gather_coords_sparse_r0_c2_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c3, gather_coords_sparse_r0_c3_v);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_sparse_r0_c4, gather_coords_sparse_r0_c4_v);
		}
	}

	switch (coords_bytes) {
		case 1:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_dense_1, gather_coords_dense_1_v);
		case 2:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_dense_2, gather_coords_dense_2_v);
		case 3:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_dense_3, gather_coords_dense_3_v);
		case 4:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_dense_4, gather_coords_dense_4_v);
	}

	return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, i_s, i_e, gather_coords_dense, gather_coords_dense_v);
...
static inline
long
decompress_and_compute_kernel_sort_diff(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long i_s, long i_e)
{
	return decompress_and_compute_kernel_sort_diff_select(buf, x, y, NULL, 0, i_s, i_e);
}
...
decompress_kernel_sort_diff(ValueType * vals, unsigned char * restrict buf, long i_s, long i_e)
{
	...
	int * rev_permutation = t_rev_permutation[tnum];
	long i, bytes;
	bytes = decompress_and_compute_kernel_sort_diff_select(buf, NULL, NULL, &num_vals, 1, i_s, i_e);
