...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals, long i_s, long j_s, unsigned char * buf, const long num_vals, long * num_vals_out)
{
	...
	*((double *) &data_intro[data_intro_bytes]) = fabs(window[0]);
	data_intro_bytes += 8;
	...
	uint64_t sign = window[0] >= 0 ? 0 : 1;
	sign <<= 7;
	data_val_lens[0] = sign;

	for (i=1;i<num_vals;i++)
	{
		// const uint64_t mask_sign = 0x8000000000000000ULL;
		// const uint64_t mask_abs = ~mask_sign;
		// uint64_t sign = ((uint64_t *) window)[i] & mask_sign;
		sign = window[i] >= 0 ? 0 : 1;
		sign <<= 7;

		// uint64_t bits_buf = 0;
		diff = fabs(window[i]) - fabs(window[i-1]);
		...
		{
			len = 53;
			data_val_lens[i] = 53 | sign;   // Special 'len' code for zero value.
		}
		else
		{
			ptr = &diff;   // C++ cancer.
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate)
{
	...
	val_abs = *((double *) &data_intro[data_intro_bytes]);
	data_intro_bytes += 8;
