...
static inline
int
bucketsort_find_bucket(int id, [[gnu::unused]] void * unused)
{
	return id;
}
...
struct DIVArray : Matrix_Format
{
	...
	DIVArray(INT_T * row_ptr, INT_T * ja, ValueTypeReference * a, long m, long n, long nnz, long symmetric, long symmetry_expanded) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a), symmetric(symmetric), symmetry_expanded(symmetry_expanded)
	{
		...
		time_compress = time_it(1,
			...
			num_packet_vals = atol(getenv("CSRCV_NUM_PACKET_VALS"));
			...
			_Pragma("omp parallel")
			{
				...
				packet = (typeof(packet)) aligned_alloc(64, max_packet_size);
				...
				while (j < j_e)
				{
					...
					size = compress_kernel_div(row_ptr, ja, &a[j], i, i_s, i_e, j, &data[pos], num_vals_max, &num_vals, this->symmetric);
				...
				long packet_reordering = atol(getenv("DIV_PACKET_REORDERING"));
				if (packet_reordering)
				{
					...
					{
						rev_permutation[k] = k;
						// bucket_id[k] = packet_type_id(&data[td->packet_group_pos[k]]);
					...
					unsigned char * data_new = (typeof(data_new)) aligned_alloc(64, td->compr_data_size);
					...
					for (k=0;k<td->num_packet_groups;k++)
					{
						...
						for (i=0;i<len;i++)
						{
							data_new[packet_group_pos_new[k] + i] = data[td->packet_group_pos[rev_permutation[k]] + i];
						}
					}
					packet_group_pos_new[td->num_packet_groups] = sum;
					if (sum != td->compr_data_size)
						error("test");

					free(td->packet_group_pos);
					td->packet_group_pos = packet_group_pos_new;
					free(td->compr_data);
					td->compr_data = data_new;
		...
		_Pragma("omp parallel")
		{
			...
			while (1)
			{
				...
				{
					size = decompress_kernel_div(&coo_ia_new[pos_decompress], &coo_ja_new[pos_decompress], &coo_a_new[pos_decompress], &num_vals, &data[pos], i_s, i_e);
	...
	~DIVArray()
	{
		...
		adapt_statistics_concurrent_base(struct adapt_statistics_data * stats)
		{
			...
			long num_packet_vals = atol(getenv("CSRCV_NUM_PACKET_VALS"));
			...
			if (num_vals_max > 0)
			{
				size = compress_kernel_div(row_ptr, ja, &a[j], i, i_s, i_e, j, &data[pos], num_vals_max, &num_vals, symmetric);
			}
			...
			for (l=0;l<test_n;l++)
			{
				time = time_it(1,
					...
					while (size_dc < size)
					{
						size_dc += decompress_and_compute_kernel_div(&(data[pos]), x_bench, y_bench, i_s, i_e);
					}
				);
				if (time < time_dc)
					time_dc = time;
		...
		td_self->time_compute += time_it(1,
			while (1)
			{
				while (1)
				{
					...
					while (pos != packet_group_pos[pg+1])
					{
						pos += decompress_and_compute_kernel_div(&(data[pos]), x, y, i_s, i_e);
					}
				}
