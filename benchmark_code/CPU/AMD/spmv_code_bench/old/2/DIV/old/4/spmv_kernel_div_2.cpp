...
steal_work(CSRCVSArrays * restrict csr, int num_threads, int tnum,
		int * tnum_target_out, long * pg_out, long * pg_e_out)
{
	...
	while (1)
	{
		...
		for (i=0;i<num_threads;i++)
		{
			if (i == tnum)
				continue;
			finished = __atomic_load_n(&td[i].finished, __ATOMIC_SEQ_CST);
			// if (tnum == 0)
				// printf("%ld : %ld %ld\n", i, finished, all_finished);
			if (finished == 1)
				continue;
			all_finished = 0;
			num_packet_groups_left = td[i].num_packet_groups - __atomic_load_n(&td[i].pg_current, __ATOMIC_SEQ_CST);
		...
		if (tnum_target >= 0)
		{
			...
			if (work <= 0)
				continue;
			pg = __atomic_fetch_add(&td[tnum_target].pg_current, work, __ATOMIC_SEQ_CST);
			if (pg >= td[tnum_target].num_packet_groups)
				continue;
			if (work > td[tnum_target].num_packet_groups - pg)
				work = td[tnum_target].num_packet_groups - pg;
			printf("tnum=%d , tnum_target=%d , pg=%ld , work=%ld\n", tnum, tnum_target, pg, work);
...
compute_div(CSRCVSArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	#pragma omp parallel
	{
		...
		int tnum = omp_get_thread_num();
		int tnum_target;
		long * pg_current_ptr = &td[tnum].pg_current;
		int * packet_group_pos = td[tnum].packet_group_pos;
		long * finished_ptr = &td[tnum].finished;
		unsigned char * data = td[tnum].compr_data;
		long pos, pg, pg_e, i, i_s, i_e;
		long all_finished;

		__atomic_store_n(finished_ptr, 0, __ATOMIC_RELAXED);
		__atomic_store_n(pg_current_ptr, 0, __ATOMIC_RELAXED);
		i_s = td[tnum].i_s;
		i_e = td[tnum].i_e;
		i = i_s;
		__atomic_store_n(&td[tnum].pg_e, td[tnum].num_packet_groups, __ATOMIC_RELAXED);
		pg_e = td[tnum].num_packet_groups;
		pos = 0;
		...
		thread_time_compute[tnum] += time_it(1,
			...
			while (1)
			{
				pg = __atomic_fetch_add(pg_current_ptr, 1, __ATOMIC_SEQ_CST);
				if (__builtin_expect(pg >= pg_e, 0))
				{
					...
					if (tnum == 0)
					{
						all_finished = steal_work(csr, num_threads, tnum, &tnum_target, &pg, &pg_e);
						if (all_finished == 1)
							break;
						__atomic_store_n(pg_current_ptr, pg, __ATOMIC_SEQ_CST);
						data = td[tnum_target].compr_data;
						packet_group_pos = td[tnum_target].packet_group_pos;
