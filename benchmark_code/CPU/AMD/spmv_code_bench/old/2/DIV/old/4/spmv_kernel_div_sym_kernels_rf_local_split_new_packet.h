...
mult_add_serial(ValueType * x_rel, ValueType * y_rel, [[gnu::unused]] ValueType * x, [[gnu::unused]] ValueType * y, ValueType val, uint64_t row_rel, uint64_t col_rel, [[gnu::unused]] uint64_t row, [[gnu::unused]] uint64_t col,
		double ** window_ptr, INT_T ** window_ia_ptr, INT_T ** window_ja_ptr, const int validate)
{
	if (validate)
	{
		**window_ptr = val;
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate,
		...
		void (* mult_add)(ValueType * x_rel, ValueType * y_rel, ValueType * x, ValueType * y, ValueType val, uint64_t row_rel, uint64_t col_rel, uint64_t row, uint64_t col, double ** window_ptr, INT_T ** window_ia_ptr, INT_T ** window_ja_ptr, const int validate),
		void (* mult_add_v)(ValueType * x_rel, ValueType * y_rel, ValueType * x, ValueType * y, vec_t(VTF, VEC_LEN) val, vec_t(i64, VEC_LEN) row_rel, vec_t(i64, VEC_LEN) col_rel, vec_t(i64, VEC_LEN) row, vec_t(i64, VEC_LEN) col, double ** window_ptr, INT_T ** window_ia_ptr, INT_T ** window_ja_ptr, const int validate)
		)
{
	int tnum = omp_get_thread_num();
	ValueType * x_rel;
