...
mult_add_vector_sym([[gnu::unused]] ValueType * x_rel, [[gnu::unused]] ValueType * y_rel, ValueType * x, ValueType * y, vec_d_t val, [[gnu::unused]] vec_i_t row_rel, [[gnu::unused]] vec_i_t col_rel, vec_i_t row, vec_i_t col)
{
	for (long iter=0;iter<vec_len_pd;iter++)
	{
		...
		y[col[iter]] += val[iter] * x[row[iter]];
	}
}
		...
		)
{
	...
	data_intro_bytes += sizeof(uint32_t);
	// printf("num_vals=%ld, num_vals_unique=%ld\n", num_vals, num_vals_unique);
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=vec_len_pd)
		{
			...
			for (j=0;j<rf;j++)
			{
				...
				vec_i_t row, col;
				row = row_rel + vec_set1_epi64(row_min);
				col = col_rel + vec_set1_epi64(col_min);

				if (validate)
					vec_storeu_pd(&window[k], val.d);
				else
				{
					mult_add_v(x_rel, y_rel, x, y, val.d, row_rel, col_rel, row, col);
				}
			}
		}
		...
		for (i=num_rf_vals_unique_multiple;i<num_rf_vals_unique;i++)
		{
			...
			for (j=0;j<rf_multiple;j+=vec_len_pd)
			{
				...
				{
					mult_add_v(x_rel, y_rel, x, y, val_buf, row_rel, col_rel, row, col);
				}
			}
			...
			for (j=rf_multiple;j<rf;j++)
			{
				...
				{
					mult_add(x_rel, y_rel, x, y, val.d[lane_id], row_rel, col_rel, row, col);
				}
			}
		}
...
decompress_and_compute_kernel_sort_diff(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e)
{
	...
	{
		packet_sym_bytes = decompress_and_compute_kernel_sort_diff_select(data, x, y, NULL, 0, mult_add_serial_sym, mult_add_vector_sym);
	}
...
static
long
decompress_kernel_sort_diff([[gnu::unused]] ValueType * vals, unsigned char * restrict buf, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e)
{
	long num_vals = 0;
	int tnum = omp_get_thread_num();
	[[gnu::unused]] double * window = t_window[tnum];
	[[gnu::unused]] int * rev_permutation = t_rev_permutation[tnum];
	unsigned char * data_intro = buf;
	uint64_t data_intro_bytes = 0;
	unsigned char * data;
	uint64_t packet_bytes = 0, packet_sym_bytes = 0;
	...
	if (data_intro_packet_sym_bytes > 0)
	{
		// printf("2 in %d\n", data_intro_packet_sym_bytes);
		packet_sym_bytes = decompress_and_compute_kernel_sort_diff_select(data, NULL, NULL, &num_vals, 1, mult_add_serial_sym, mult_add_vector_sym);
		// printf("2 out\n");
	}
	data += packet_sym_bytes;
