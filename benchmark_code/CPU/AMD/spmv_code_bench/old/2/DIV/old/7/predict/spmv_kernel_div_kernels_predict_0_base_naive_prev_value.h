...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals_unpadded, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	long i, i_block_e, i_lane_block_s, i_lane_block_e, j, k, l;
		...
		)
{
	...
	for (i=4;i<num_vals_padded;i+=4)
	{
		...
		// sign = ~sign;
		sign = _mm256_xor_si256(sign, _mm256_cmpeq_epi32(sign, sign));   // There is no NOT, so we have to do it via xor with '0xFFF...'.
