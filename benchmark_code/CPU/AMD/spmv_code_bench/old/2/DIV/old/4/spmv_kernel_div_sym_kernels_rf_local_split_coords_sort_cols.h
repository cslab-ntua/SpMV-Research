...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, long i_t_s, long i_t_e, long j_s, unsigned char * buf, long num_vals_suggested, long * num_vals_out)
{
	...
	/* Sort the symmetric values by column index. */
	for (k=0;k<num_vals_sym;k++)
		rev_permutation[k] = k;
	compare_data.vals = window_sym;
	compare_data.cols = cols_sym;
	compare_data.rows = rows_sym;
	quicksort_sym(rev_permutation, num_vals_sym, &compare_data, t_qsort_partitions[tnum]);
	for (k=0;k<num_vals_sym;k++)
	{
		window[k] = window_sym[rev_permutation[k]];
		rows[k] = rows_sym[rev_permutation[k]];
		cols[k] = cols_sym[rev_permutation[k]];
	}
	for (k=0;k<num_vals_sym;k++)
	{
		window_sym[k] = window[k];
		rows_sym[k] = rows[k];
		cols_sym[k] = cols[k];
	}
	...
	*((uint32_t *) &data_intro[data_intro_bytes]) = num_rfs;
	...
	unsigned char * data_coords_sym = &data_coords[data_coords_bytes];
	const uint64_t data_coords_sym_bytes = coords_bytes * num_vals_sym;

	unsigned char * data_val_lens = &data_coords_sym[data_coords_sym_bytes];
	...
	{
		row_diff = rows_sym[i] - row_min;
		col_diff = cols_sym[i] - col_min;
		coords = row_diff | (col_diff << row_bits);
		memcpy(&data_coords_sym[i*coords_bytes], &coords, coords_bytes);
	}
	...
	return data_intro_bytes + data_rfs_bytes + data_num_unique_vals_per_rf_bytes + data_coords_bytes + data_coords_sym_bytes + data_val_lens_bytes + data_val_lanes_bytes + sizeof(double) * num_vals_sym;
}
		...
		)
{
	...
	if (!validate)
	{
		if (num_vals_sym > 0)
		{
			...
			// uint64_t row_rel, col_rel;
			// uint64_t row_cur, row, col;
			// double val, sum = 0;
			// gather_coords(i, data_coords_sym, coords_bytes, row_bits, col_bits, &row_rel, &col_rel);
			// row_cur = row_rel + row_min;
			// for (i=0;i<num_vals_sym;i++)
			// {
				// gather_coords(i, data_coords_sym, coords_bytes, row_bits, col_bits, &row_rel, &col_rel);
				// val = data_vals_sym[i];
			...
			uint64_t row_rel, col_rel;
			uint64_t col_cur, row, col;
			double val, sum = 0;
			gather_coords(i, data_coords_sym, coords_bytes, row_bits, col_bits, &row_rel, &col_rel);
			col_cur = row_rel + row_min;
			for (i=0;i<num_vals_sym;i++)
			{
				gather_coords(i, data_coords_sym, coords_bytes, row_bits, col_bits, &row_rel, &col_rel);
				val = data_vals_sym[i];
