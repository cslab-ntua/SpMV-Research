...
struct packet_header {
	...
	uint32_t num_rfs;
	int data_val_lanes_size[VEC_LEN];
};
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_rf_vals_unique_multiple;i+=VEC_LEN)
		{
			...
			for (j=0;j<rf;j++)
			{
				...
				if (validate)
				{
					// for (long _i=0;_i<2;_i++)
					// {
						// _mm256_storeu_pd((double*)((double*)(&(((__m256d_f64_2*)(&window[k]))->v[_i]))), (val.d).v[_i]);
					// }
...
decompress_and_compute_kernel_sort_diff(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e)
{
	...
	long i;
	get_packet_rows(buf, &packet_i_s, &packet_i_e);
	for (i=packet_i_s+1;i<packet_i_e;i++)
	{
		y[i] = 0;
	}
