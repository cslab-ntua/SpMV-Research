...
struct CSRCVSArrays : Matrix_Format
{
	...
	CSRCVSArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		tds = (typeof(tds)) aligned_alloc(64, num_tasks * sizeof(*tds));

		thread_time_compute = (typeof(thread_time_compute)) malloc(num_tasks * sizeof(*thread_time_compute));
		...
		time_balance = time_it(1,
			_Pragma("omp parallel")
			{
				...
				loop_partitioner_balance_prefix_sums(num_tasks, task_num, row_ptr, m, nnz, &tds[task_num].i_s, &tds[task_num].i_e);
			}
		);
		...
		time_compress = time_it(1,
			...
			_Pragma("omp parallel")
			{
				...
				i_s = tds[task_num].i_s;
				i_e = tds[task_num].i_e;
				j_s = row_ptr[i_s];
				j_e = row_ptr[i_e];
				...
				while (1)
				{
					...
					// if (task_num == 0)
						// printf("%d: i=[%ld,%ld] , j=%ld[%ld,%ld] , p=%ld , num_vals=%ld , packet_i=[%d, %d] (%d) , t_nnz=%ld\n",
								// task_num, i_s, i_e, j, j_s, j_e, p, num_vals, packet_i_s[p], packet_i_e[p], packet_i_e[p] - packet_i_s[p], t_nnz);
				...
				if (num_vals_total_dc != num_vals_total)
					error("task_num=%d: num_vals_total_dc = %ld != num_vals_total = %ld", task_num, num_vals_total_dc, num_vals_total);
				num_packets = p;
				tds[task_num].compr_data_size = pos;
				tds[task_num].compr_data = data;
				tds[task_num].num_packets = num_packets;
				tds[task_num].num_vals = num_vals_total;

				tds[task_num].num_packet_groups = num_packet_groups;
				long tmp, sum;
				sum = 0;
				...
				tds[task_num].packet_group_pos = (typeof(&(*tds[task_num].packet_group_pos))) malloc((num_packet_groups+1) * sizeof(*tds[task_num].packet_group_pos));
				for (k=0;k<num_packet_groups+1;k++)
					tds[task_num].packet_group_pos[k] = packet_group_pos[k];
		...
		{
			compr_data_size += tds[i].compr_data_size;
			compr_data_size += (tds[i].num_packet_groups + 1) * sizeof(*tds[i].packet_group_pos);
...
static inline
long
steal_sched_local_only(struct thread_data * tds, [[gnu::unused]] int num_tasks, long task_num,
		long * task_num_target_out, long * work_out)
{
	...
	*task_num_target_out = task_num_target;
	*work_out = work_max;
	return all_finished;
}
...
static inline
long
steal_sched_default(struct thread_data * tds, int num_tasks, long task_num,
		long * task_num_target_out, long * work_out)
{
	struct thread_data * td_target;
	long finished, all_finished;
...
static inline
long
steal_work(CSRCVSArrays * restrict csr, int num_tasks, long task_num, long * flag_local_work_finished_ptr)
{
	struct thread_data * tds = csr->tds;
	struct thread_data * td_self = &tds[task_num];
	struct thread_data * td_target;
	long task_num_owner;
	...
	[[gnu::unused]] long task_mem_node_num = topotk_get_task_mem_node_num();
	...
	while (1)
	{
		...
		{
			long local_finished = steal_sched_local_only(tds, num_tasks, task_num, &task_num_target, &work_target);
			if (local_finished)
			{
		...
		{
			all_finished = steal_sched_default(tds, num_tasks, task_num, &task_num_target, &work_target);
		}

		if (all_finished)
			return 1;

		if (task_num_target < 0)
			continue;
		...
		td_target = &tds[task_num_target];
		if (__atomic_exchange_n(&td_target->steal_lock, 1, __ATOMIC_SEQ_CST))
			continue;

		task_num_owner = td_target->task_num_owner;
		...
		pg_e = pg + work;
		// printf("task_num=%ld, task_mem_node_num=%ld, task_num_target=%ld, task_num_owner=%ld, task_mem_node_num_owner=%ld, pg=%ld, pg_e=%ld, work_target=%ld, work=%ld\n", task_num, task_mem_node_num, task_num_target, task_num_owner, task_mem_node_num_owner, pg, pg_e, work_target, work);
		__atomic_store_n(&td_self->task_num_owner, task_num_owner, __ATOMIC_SEQ_CST);
...
compute_div(CSRCVSArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	#pragma omp parallel
	{
		...
		__atomic_store_n(&td_self->task_num_owner, task_num, __ATOMIC_RELAXED);
		...
		thread_time_compute[task_num] += time_it(1,
			while (1)
			{
				while (1)
				{
					...
					// long packet_i_s, packet_i_e;
					// get_packet_rows(&(data[pos]), &packet_i_s, &packet_i_e);
					// y[packet_i_s] = 0;
				...
				all_finished = steal_work(csr, num_tasks, task_num, &flag_local_work_finished);
				if (all_finished == 1)
					break;
				td_owner = &tds[td_self->task_num_owner];
				pg_e = td_self->pg_e;
