	...
	CSRCVSArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		time_balance = time_it(1,
			_Pragma("omp parallel")
			{
				...
				_Pragma("omp single")
				{
					l3_size_total = num_task_l3_nodes * l3_size;
				}
			}
		);
		...
		time_compress = time_it(1,
			...
			_Pragma("omp parallel")
			{
				...
				long num_vals, num_vals_max, num_vals_dc, num_vals_total=0, num_vals_total_dc=0;
				long num_packets;
				long num_packet_groups;
				long pos, size;
				long upper_boundary;
				...
				packet_group_pos = (typeof(packet_group_pos)) malloc(t_nnz * sizeof(*packet_group_pos));   // Worst case t_nnz packet groups.
				...
				data = (typeof(data)) aligned_alloc(64, max_data_size);
					...
					if (num_vals_max > 0)
					{
						size = compress_kernel_sort_diff(row_ptr, ja, &a[j], i, i_s, i_e, j, &data[pos], num_vals_max, &num_vals);
					}
					omp_thread_reduce_global(reduce_add, size, 0, 0, 0, , &size_accum);
					omp_thread_reduce_global(reduce_add, num_vals, 0, 0, 0, , &num_vals_accum);

					double predicted_total_size = ((double) size_accum) / num_vals_accum * nnz;
					td->predicted_total_size = predicted_total_size;

					_Pragma("omp single")
					{
						printf("predicted_total_size = %lf , l3_size_total = %lf\n", predicted_total_size / 1024 / 1024, ((double) l3_size_total) / 1024 / 1024);
						if (predicted_total_size < 2 * l3_size_total)
							DIV_ENABLED = 0;
						else
							DIV_ENABLED = 1;
						RF_ENABLED = 1;
						printf("DIV_ENABLED=%ld , RF_ENABLED=%ld\n", DIV_ENABLED, RF_ENABLED);
					}
				#endif
				...
				td->num_packet_groups = num_packet_groups;
				long tmp, sum;
				sum = 0;
				...
				td->packet_group_pos = (typeof(td->packet_group_pos)) malloc((num_packet_groups+1) * sizeof(*td->packet_group_pos));
				for (k=0;k<num_packet_groups+1;k++)
					td->packet_group_pos[k] = packet_group_pos[k];
	...
	#pragma omp parallel
	{
		...
		for (i=0;i<m;i++)
		{
			...
			for (j=csr_row_ptr_new[i];j<csr_row_ptr_new[i+1];j++)
			{
				...
				if (csr_a_new[j] != a[j])
					printf("csr_a_new[%ld]=%g, a[%ld]=%g   (row=%ld, col=%d)\n", j, csr_a_new[j], j, a[j], i, ja[j]);
			}
		}
...
statistics_print_labels(char * buf, long buf_n)
{
	...
	len += snprintf(buf + len, buf_n - len, ",%s", "tolerance");
		...
		len += snprintf(buf + len, buf_n - len, ",%s", "DIV_ENABLED");
...
CSRCVSArrays::statistics_print_data(char * buf, long buf_n)
{
	...
	len += snprintf(buf + len, buf_n - len, ",%g",  tolerance);
		...
		len += snprintf(buf + len, buf_n - len, ",%ld", DIV_ENABLED);
