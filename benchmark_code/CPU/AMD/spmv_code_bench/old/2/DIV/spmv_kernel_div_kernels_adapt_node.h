...
struct thread_data_kernel {
	struct node * window_node;
	struct node * window_node_buf;
	ValueType * window;
...
extern "C"{
	...
	#define QUICKSORT_GEN_TYPE_3  struct node
...
static inline
int
quicksort_cmp(int a, int b, struct node * aux)
{
	ValueType va=aux[a].val, vb=aux[b].val;
	int ca=aux[a].col, cb=aux[b].col;
	int ret;
	if (va*vb <= 0)
...
quicksort_cmp(int a, int b, struct node * aux)
{
	int ra=aux[a].row, rb=aux[b].row;
	int ca=aux[a].col, cb=aux[b].col;
	int ret;
	ret = (ra > rb) ? 1 : (ra < rb) ? -1 : (ca > cb) ? 1 : (ca < cb) ? -1 : 0;
	return ret;
}
...
compress_init_sort_diff(__attribute__((unused)) ValueType * vals, __attribute__((unused)) const long num_vals, long packet_size)
{
	...
	_Pragma("omp parallel")
	{
		...
		__aligned_alloc(tdk->window_node, window_size);
		__aligned_alloc(tdk->window_node_buf, window_size);
...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	struct node * window_node = tdk->window_node;
	struct node * window_node_buf = tdk->window_node_buf;
	...
	{
		rev_permutation[k] = k;
		window_node[k].val = vals[k];
		window_node[k].row = rows[k];
		window_node[k].col = cols[k];
	}
	quicksort_vals(rev_permutation, num_vals, window_node, tdk->qsort_partitions);
	...
	if (!div_enabled)
	{
		...
		if (num_rf_vals > 0)
		{
			...
			quicksort_csr(rev_permutation, num_rf_vals, window_node, tdk->qsort_partitions);
			for (k=0;k<num_vals;k++)
			{
				window_buf[k] = window[rev_permutation[k]];
