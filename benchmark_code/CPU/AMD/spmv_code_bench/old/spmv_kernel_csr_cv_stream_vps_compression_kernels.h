...
static uint64_t window_size_max;
...
uint64_t row_bits_export;
uint64_t col_bits_export;
...
void
compress_init_sort_diff(const long packet_size_max)
{
	int num_threads = omp_get_max_threads();
	get_num_bits_of_binary_representation(packet_size_max - 1, &window_size_bits, &window_size_max);    // Maximum representable number we want is 'packet_size_max - 1'.
	printf("window_size_max = %ld , window_size_bits = %ld\n", window_size_max, window_size_bits);
	...
	#pragma omp parallel
	{
		...
		t_window[tnum] = (typeof(&(**t_window))) malloc(window_size_max * sizeof(**t_window));
		t_rows_diff[tnum] = (typeof(&(**t_rows_diff))) malloc(window_size_max * sizeof(**t_rows_diff));
		t_cols[tnum] = (typeof(&(**t_cols))) malloc(window_size_max * sizeof(**t_cols));
		t_permutation[tnum] = (typeof(&(**t_permutation))) malloc(window_size_max * sizeof(**t_permutation));
		t_rev_permutation[tnum] = (typeof(&(**t_rev_permutation))) malloc(window_size_max * sizeof(**t_rev_permutation));
		t_qsort_partitions[tnum] = (typeof(&(**t_qsort_partitions))) malloc(window_size_max * sizeof(**t_qsort_partitions));
...
static inline
long
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals, long i_s, long j_s, unsigned char * buf, const long num_vals_remaining, long * num_vals_out)
{
	long num_vals;
	int tnum = omp_get_thread_num();
	...
	row_bits_export = row_bits;
	col_bits_export = col_bits;
