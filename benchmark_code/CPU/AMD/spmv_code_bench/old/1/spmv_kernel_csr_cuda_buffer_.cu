...
struct CSRArrays : Matrix_Format
{
	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		cudaMalloc(&row_ptr_dev, (m+1) * sizeof(*row_ptr_dev));
		cudaMemcpy(row_ptr_dev, row_ptr, (m+1) * sizeof(*row_ptr_dev), cudaMemcpyHostToDevice);
...
__global__ void gpu_kernel_csr_basic(INT_T * thread_block_i_s, INT_T * thread_block_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y, unsigned char * rel_row_idx, int row_cluster_size)
{
	...
	for (k=i_s;k<i_e;k+=row_cluster_size)
	{
		...
		j_s = row_ptr[k];
		j_e = row_ptr[k+i_rel_e];
		for (j=j_s+tidb;j<j_e;j+=block_size)
		{
			i_rel = rel_row_idx[j];
			val_buf[i_rel*block_size + tidb] += a[j] * x[ja[j]];
			// val_buf[i_rel*block_size + tidb] = __fma_rz(a[j], x[ja[j]], val_buf[i_rel*block_size + tidb]);
		}
		...
		for (i_rel=tidb;i_rel<i_rel_e;i_rel+=block_size)
		{
			...
			{
				sum += val_buf[i_rel*block_size + j];
				val_buf[i_rel*block_size + j] = 0;
			}
			y[k + i_rel] = sum;
		}
		...
		// {
			...
			// {
				// if (tidb % (2*j) == 0)
				// {
					// val_buf[i_rel*block_size + tidb] += val_buf[i_rel*block_size + tidb + j];
					// val_buf[i_rel*block_size + tidb + j] = 0;
				// }
				// __syncthreads();
			// }
			// if (tidb == 0)
			// {
				// y[k + i_rel] = val_buf[i_rel*block_size];
				// val_buf[i_rel*block_size] = 0;
			// }
		// }
...
compute_csr(CSRArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	gpu_kernel_csr_basic<<<grid_dims, block_dims, (csr->row_cluster_size*block_size*sizeof(ValueType))>>>(thread_block_i_s_dev, thread_block_i_e_dev, csr->row_ptr_dev, csr->ja_dev, csr->a_dev, csr->x_dev, csr->y_dev, csr->rel_row_idx_dev, csr->row_cluster_size);
	// gpu_kernel_csr_flat<<<grid_dims, block_dims>>>(thread_block_i_s_dev, thread_block_i_e_dev, csr->row_ptr_dev, csr->ja_dev, csr->a_dev, csr->x_dev, csr->y_dev);
