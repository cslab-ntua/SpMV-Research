	...
	CSRArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		time_balance = time_it(1,
			for (i=0;i<num_threads;i++)
			{
				...
				loop_partitioner_balance_prefix_sums(num_threads, i, row_ptr, m, nnz, &thread_i_s[i], &thread_i_e[i]);
			}
		);
...
__global__ void gpu_kernel_csr_basic(INT_T * thread_i_s, INT_T * thread_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
...
__global__ void gpu_kernel_csr_flat(INT_T * thread_i_s, INT_T * thread_i_e, INT_T * row_ptr, INT_T * ja, ValueType * a, ValueType * restrict x, ValueType * restrict y)
{
	int tgid = cuda_get_thread_num();
	long i, i_s, i_e, j, j_e;
	ValueType sum;
...
compute_csr(CSRArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	gpu_kernel_csr_basic<<<grid_dims, block_dims>>>(thread_i_s_dev, thread_i_e_dev, csr->row_ptr_dev, csr->ja_dev, csr->a_dev, csr->x_dev, csr->y_dev);
	// gpu_kernel_csr_flat<<<grid_dims, block_dims>>>(thread_i_s_dev, thread_i_e_dev, csr->row_ptr_dev, csr->ja_dev, csr->a_dev, csr->x_dev, csr->y_dev);
