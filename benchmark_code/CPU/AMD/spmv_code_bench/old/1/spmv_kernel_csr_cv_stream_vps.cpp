...
extern "C"{
	...
	#include "spmv_kernel_csr_cv_stream_vps_compression_kernels.h"
#ifdef __cplusplus
}
#endif
...
extern uint64_t row_bits_export;
extern uint64_t col_bits_export;
...
static inline
void
compress_init(__attribute__((unused)) ValueType * vals, __attribute__((unused)) const long num_vals, const long packet_size)
{
	compress_init_sort_diff(packet_size);
}
...
struct CSRCVSArrays : Matrix_Format
{
	...
	CSRCVSArrays(INT_T * ia, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), ia(ia), ja(ja), a(a)
	{
		...
		time_compress = time_it(1,
			long num_packet_vals_max;
			num_packet_vals_max = get_num_uncompressed_packet_vals();
			compress_init(a, nnz, num_packet_vals_max);
			_Pragma("omp parallel")
			{
				int tnum = omp_get_thread_num();
				unsigned char * data;
				...
				for (j=j_s;j<j_e;)
				{
					...
					size = compress(ia, ja, &a[j], i, j, &data[pos], j_e - j, &num_vals);
...
csr_to_format(INT_T * row_ptr, INT_T * col_ind, ValueType * values, long m, long n, long nnz)
{
	...
	csr->format_name = (char *) "CSR_CVS_VPS";
	return csr;
}
...
int
CSRCVSArrays::statistics_print(__attribute__((unused)) char * buf, __attribute__((unused)) long buf_n)
{
	int num_threads = omp_get_max_threads();
	...
	len = 0;
	len += snprintf(buf + len, buf_n - len, ",%.2lf", (time_max - time_avg) / time_avg * 100);
	len += snprintf(buf + len, buf_n - len, ",%.2lf",  (data_size_max - data_size_avg) / data_size_avg * 100);
	return len;
}
