...
static inline
long
select_num_vals(INT_T * ia, long i_s, long j_s, long num_vals)
{
	long i_e, j_e;
	long num_rows;
...
static inline
double
compress_diff_lossy(double val, double val_prev, double diff, double tolerance)
{
	...
	{
		bits.u &= ~mask_inv;
		val_new = val_prev + bits.d;
		if (fabs(val - val_new) / fabs(val) > tolerance)
			break;
		pow2 <<= 1;
		mask_inv = pow2 - 1;
	}
...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals_unpadded, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	uint64_t trailing_zero_bytes = 0;
	uint64_t len = 0;
	double diff = 0;
	union {
		double d;
	...
	num_vals = select_num_vals(ia, i_s, j_s, num_vals);
	...
	*((double *) &data_intro[data_intro_bytes]) = window[3];
	...
	for (i=4;i<num_vals_padded;i++)
	{
		...
		// diff = compress_diff_lossy(window[i], val_prev[i%4], diff, 1.0e-8);
		...
		{
			/* Differences  can actually be negative due to the rounding errors!
			 */
			bits.d = diff;
			trailing_zeros = (bits.u == 0) ? 64 : __builtin_ctzl(bits.u);
			trailing_zero_bytes = trailing_zeros >> 3;
...
{
	const __m256i row_bits_mask = _mm256_set1_epi64x((1<<row_bits) - 1);   // here '-' has precedence over '<<'!
	// const __m256i col_bits_mask = _mm256_set1_epi64x((1<<col_bits) - 1);
	__m256i coords;
	...
	*row_rel_out = coords & row_bits_mask;
	...
	// row_rel_out = _mm256_set_epi64x(bits_extract_64(coords[3], 0, row_bits), bits_extract_64(coords[2], 0, row_bits), bits_extract_64(coords[1], 0, row_bits), bits_extract_64(coords[0], 0, row_bits));
	*col_rel_out = _mm256_set_epi64x(bits_extract_64(coords[3], row_bits, col_bits), bits_extract_64(coords[2], row_bits, col_bits), bits_extract_64(coords[1], row_bits, col_bits), bits_extract_64(coords[0], row_bits, col_bits));
}


static inline
void
gather_coords_sparse(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out)
{
	// const uint64_t row_bytes = row_bits >> 3;
	// const __m256i row_bits_mask = _mm256_set1_epi64x((1<<row_bits) - 1);   // here '-' has precedence over '<<'!
	const __m256i col_bits_mask = _mm256_set1_epi64x((1<<col_bits) - 1);
	...
	// row_rel = _mm256_set_epi64x(*((uint64_t *) &data_coords[(i+3)*coords_bytes]), *((uint64_t *) &data_coords[(i+2)*coords_bytes]), *((uint64_t *) &data_coords[(i+1)*coords_bytes]), *((uint64_t *) &data_coords[i*coords_bytes]));
	// *row_rel_out = row_rel & row_bits_mask;
	...
	// col_rel = _mm256_set_epi64x(*((uint64_t *) &data_coords[(i+3)*coords_bytes + row_bytes]), *((uint64_t *) &data_coords[(i+2)*coords_bytes + row_bytes]), *((uint64_t *) &data_coords[(i+1)*coords_bytes + row_bytes]), *((uint64_t *) &data_coords[i*coords_bytes + row_bytes]));
...
static inline
void
gather_coords_sparse_1(long i, unsigned char * data_coords, const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out)
{
	__m256i row_rel, col_rel;
	row_rel = _mm256_set_epi64x(data_coords[(i+3)*coords_bytes], data_coords[(i+2)*coords_bytes], data_coords[(i+1)*coords_bytes], data_coords[i*coords_bytes]);
...
static inline
void
gather_coords_sparse_2(long i, unsigned char * data_coords, const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out)
{
	__m256i row_rel, col_rel;
	row_rel = _mm256_set_epi64x(data_coords[(i+3)*coords_bytes], data_coords[(i+2)*coords_bytes], data_coords[(i+1)*coords_bytes], data_coords[i*coords_bytes]);
...
static inline
void
gather_coords_sparse_3(long i, unsigned char * data_coords, const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out)
{
	const __m256i col_bits_mask = _mm256_set1_epi64x((1<<24) - 1);
	__m256i row_rel, col_rel;
	row_rel = _mm256_set_epi64x(data_coords[(i+3)*coords_bytes], data_coords[(i+2)*coords_bytes], data_coords[(i+1)*coords_bytes], data_coords[i*coords_bytes]);
	col_rel = _mm256_set_epi64x(*((uint64_t *) &data_coords[(i+3)*coords_bytes + 1]), *((uint64_t *) &data_coords[(i+2)*coords_bytes + 1]), *((uint64_t *) &data_coords[(i+1)*coords_bytes + 1]), *((uint64_t *) &data_coords[i*coords_bytes + 1]));
	col_rel = col_rel & col_bits_mask;
...
static inline
void
gather_coords_sparse_4(long i, unsigned char * data_coords, const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out)
{
	__m256i row_rel, col_rel;
	row_rel = _mm256_set_epi64x(data_coords[(i+3)*coords_bytes], data_coords[(i+2)*coords_bytes], data_coords[(i+1)*coords_bytes], data_coords[i*coords_bytes]);
		...
		void (* gather_coords)(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out))
{
	...
	// col_rel = _mm256_set_epi64x(bits_extract_64(coords[3], row_bits, col_bits), bits_extract_64(coords[2], row_bits, col_bits), bits_extract_64(coords[1], row_bits, col_bits), bits_extract_64(coords[0], row_bits, col_bits));
	...
	else
	{
		...
		// y_buf[0][row_rel[0]] += val[0] * x_rel[col_rel[0]];
		// y_buf[1][row_rel[1]] += val[1] * x_rel[col_rel[1]];
		// y_buf[2][row_rel[2]] += val[2] * x_rel[col_rel[2]];
		// y_buf[3][row_rel[3]] += val[3] * x_rel[col_rel[3]];
	...
	for (i=4;i<num_vals_padded;i+=4)
	{
		...
		// row_rel = _mm256_set_epi64x(bits_extract_64(coords[3], 0, row_bits), bits_extract_64(coords[2], 0, row_bits), bits_extract_64(coords[1], 0, row_bits), bits_extract_64(coords[0], 0, row_bits));
		col_rel = _mm256_set_epi64x(bits_extract_64(coords[3], row_bits, col_bits), bits_extract_64(coords[2], row_bits, col_bits), bits_extract_64(coords[1], row_bits, col_bits), bits_extract_64(coords[0], row_bits, col_bits)); */
...
decompress_and_compute_kernel_sort_diff_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate)
{
	...
	{
		// return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse);
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_1);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_2);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_3);
			// case 32:
			default:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_4);
			// default:
				// return 0;
		}
	}
	...
	// error("col_bits can't be more than 32: %ld", col_bits);
	// *num_vals_out = 0;
	// return 0;
}
