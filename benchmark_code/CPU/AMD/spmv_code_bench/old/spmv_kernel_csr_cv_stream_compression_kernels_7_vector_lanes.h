...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate)
{
	...
	// coords = _mm256_set_epi64x(*((uint64_t *) &data_coords[idx_coords[3]]), *((uint64_t *) &data_coords[idx_coords[2]]), *((uint64_t *) &data_coords[idx_coords[1]]), *((uint64_t *) &data_coords[idx_coords[0]]));
	...
	for (i=4;i<num_vals_padded;i+=4)
	{
		...
		// sum1 = len0 + bs_offset;
		// idx = _mm256_set_epi64x(sum1, sum1, sum1, bs_offset);
		// idx = _mm256_add_epi64(idx, _mm256_set_epi64x(len2+len1, len1, 0, 0));
		// bs_offset = idx[3] + len3;
		...
		// bits.u = _mm256_set_epi64x(*((uint64_t *) &bytestream[idx[3]]), *((uint64_t *) &bytestream[idx[2]]), *((uint64_t *) &bytestream[idx[1]]), *((uint64_t *) &bytestream[idx[0]]));
		...
		// printf("%u\n", data_val_lanes[0][0]);
		// data_val_lanes[0] += data_val_lens[i+0];
		// data_val_lanes[1] += data_val_lens[i+1];
		// data_val_lanes[2] += data_val_lens[i+2];
		// data_val_lanes[3] += data_val_lens[i+3];
