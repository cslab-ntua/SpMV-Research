	...
	CSRCVSArrays(INT_T * row_ptr, INT_T * ja, ValueType * a, long m, long n, long nnz) : Matrix_Format(m, n, nnz), row_ptr(row_ptr), ja(ja), a(a)
	{
		...
		time_balance = time_it(1,
			_Pragma("omp parallel")
			{
				...
				tds[tnum].task_num = task_num;
				tds[tnum].num_tasks = num_tasks;
				tds[tnum].task_l3_node_num = task_l3_node_num;
				tds[tnum].num_task_l3_nodes = num_task_l3_nodes;
				tds[tnum].task_mem_node_num = task_mem_node_num;
				tds[tnum].num_task_mem_nodes = num_task_mem_nodes;
				tds[tnum].i_s = i_s;
				tds[tnum].i_e = i_e;
			}
		);
		...
		time_compress = time_it(1,
			...
			_Pragma("omp parallel")
			{
				...
				num_packets = p;
				tds[tnum].compr_data_size = pos;
				tds[tnum].compr_data = data;
				tds[tnum].num_packets = num_packets;
				tds[tnum].num_vals = num_vals_total;

				tds[tnum].num_packet_groups = num_packet_groups;
				long tmp, sum;
				sum = 0;
				...
				tds[tnum].packet_group_pos = (typeof(&(*tds[tnum].packet_group_pos))) malloc((num_packet_groups+1) * sizeof(*tds[tnum].packet_group_pos));
				for (k=0;k<num_packet_groups+1;k++)
					tds[tnum].packet_group_pos[k] = packet_group_pos[k];
