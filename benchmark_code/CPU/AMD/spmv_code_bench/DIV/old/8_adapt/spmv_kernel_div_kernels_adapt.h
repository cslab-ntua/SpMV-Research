...
static inline
long
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueTypeReference * vals, long i_s, long i_t_s, long i_t_e, long j_s, unsigned char * buf, long num_vals, long * num_vals_out, long symmetric)
{
	int tnum = omp_get_thread_num();
	struct thread_data_kernel * tdk = tdks[tnum];
	long num_vals_sym;
	...
	// long num_vals_consumed = select_num_vals(row_ptr, row_min, j_s, num_vals, force_row_index_lte_1_byte);
	num_vals_consumed = select_num_vals_complete_last_row(row_ptr, row_min, j_s, num_vals, force_row_index_lte_1_byte);
	if (num_vals_consumed != 0)
		num_vals = num_vals_consumed;
	else
	{
...
// void
// mult_add_vector(ValueType * x_rel, ValueType * y_rel, vec_t(VTF, VEC_LEN) val, vec_t(i64, VEC_LEN) row_rel, vec_t(i64, VEC_LEN) col_rel)
// {
	// for (long iter=0;iter<VEC_LEN;iter++)
		...
		)
{
	...
	// if (row_bits == 8)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r1_c1, gather_coords_sparse_r1_c1_v, mult_add, mult_add_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r1_c2, gather_coords_sparse_r1_c2_v, mult_add, mult_add_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r1_c3, gather_coords_sparse_r1_c3_v, mult_add, mult_add_v);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r1_c4, gather_coords_sparse_r1_c4_v, mult_add, mult_add_v);
		}
	}
	...
	if (row_bits == 0)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r0_c1, gather_coords_sparse_r0_c1_v, mult_add, mult_add_v);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r0_c2, gather_coords_sparse_r0_c2_v, mult_add, mult_add_v);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r0_c3, gather_coords_sparse_r0_c3_v, mult_add, mult_add_v);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_sparse_r0_c4, gather_coords_sparse_r0_c4_v, mult_add, mult_add_v);
		}
	}

	switch (coords_bytes) {
		case 1:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_dense_1, gather_coords_dense_1_v, mult_add, mult_add_v);
		case 2:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_dense_2, gather_coords_dense_2_v, mult_add, mult_add_v);
		case 3:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_dense_3, gather_coords_dense_3_v, mult_add, mult_add_v);
		case 4:
			return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_dense_4, gather_coords_dense_4_v, mult_add, mult_add_v);
	}

	return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, dv_enabled, gather_coords_dense, gather_coords_dense_v, mult_add, mult_add_v);
}
...
decompress_and_compute_kernel_sort_diff(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e)
{
	...
	{
		if (symmetric)
			return decompress_and_compute_kernel_sort_diff_select(buf, x, y, NULL, 0, 0, mult_add_serial_sym, mult_add_vector_sym);
		else
			return decompress_and_compute_kernel_sort_diff_select(buf, x, y, NULL, 0, 0, mult_add_serial, mult_add_vector);
	}
	else if (dv_enabled && rf_enabled)
	{
		if (symmetric)
			return decompress_and_compute_kernel_sort_diff_select(buf, x, y, NULL, 0, 1, mult_add_serial_sym, mult_add_vector_sym);
		else
			return decompress_and_compute_kernel_sort_diff_select(buf, x, y, NULL, 0, 1, mult_add_serial, mult_add_vector);
	}
...
decompress_kernel_sort_diff(INT_T * ia_out, INT_T * ja_out, ValueType * a_out, long * num_vals_out, unsigned char * restrict buf, [[gnu::unused]] long i_t_s, [[gnu::unused]] long i_t_e)
{
	...
	{
		if (symmetric)
			bytes = decompress_and_compute_kernel_sort_diff_select(buf, NULL, NULL, &num_vals, 1, 0, mult_add_serial_sym, mult_add_vector_sym);
		else
			bytes = decompress_and_compute_kernel_sort_diff_select(buf, NULL, NULL, &num_vals, 1, 0, mult_add_serial, mult_add_vector);
	}
	else if (dv_enabled && rf_enabled)
	{
		if (symmetric)
			bytes = decompress_and_compute_kernel_sort_diff_select(buf, NULL, NULL, &num_vals, 1, 1, mult_add_serial_sym, mult_add_vector_sym);
		else
			bytes = decompress_and_compute_kernel_sort_diff_select(buf, NULL, NULL, &num_vals, 1, 1, mult_add_serial, mult_add_vector);
	}
	else
	{
		error("bad packet type : dv_enabled=%ld rf_enabled=%ld", dv_enabled, rf_enabled);
