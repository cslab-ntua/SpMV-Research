...
steal_work(CSRCVSArrays * restrict csr, int num_threads, int tnum, int task_num)
{
	...
	while (1)
	{
		...
		num_finished = __atomic_load_n(&finished_threads.val, __ATOMIC_SEQ_CST);
...
compute_div(CSRCVSArrays * restrict csr, ValueType * restrict x, ValueType * restrict y)
{
	...
	#pragma omp parallel
	{
		...
		tnum_owner = tnum;
		__atomic_store_n(&td_self->tnum_owner, tnum_owner, __ATOMIC_RELAXED);
		...
		thread_time_compute[tnum] += time_it(1,
			while (1)
			{
				...
				__atomic_fetch_add(&finished_threads.val, 1, __ATOMIC_SEQ_CST);
