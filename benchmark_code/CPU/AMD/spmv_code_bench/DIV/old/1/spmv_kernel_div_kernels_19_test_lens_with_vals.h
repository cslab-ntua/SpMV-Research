...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals_unpadded, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	*((double *) &data_intro[data_intro_bytes]) = window[3];
	...
	for (i=4;i<num_vals_padded;i++)
	{
		...
		{
			len = 0;
			bytestream_write_unsafe_cast(&Bs_vals[i%4], len, 1);
		}
		else
		{
	...
	return data_intro_bytes + data_coords_bytes + data_val_lanes_bytes;
}
		...
		)
{
	...
	data_val_lanes[0] = (unsigned char *) &data_coords[data_coords_bytes];
	...
	data_val_lanes_ptr = _mm256_loadu_si256((__m256i *) data_val_lanes);
	for (i=4;i<num_vals_padded;i+=4)
	{
		gather_coords(i, data_coords, coords_bytes, row_bits, col_bits, &row_rel, &col_rel);

		// _mm256_storeu_si256((__m256i *) data_val_lanes, data_val_lanes_ptr);
		// len = _mm256_set_epi64x(*data_val_lanes[3], *data_val_lanes[2], *data_val_lanes[1], *data_val_lanes[0]);
		len = _mm256_set_epi64x(*((unsigned char *) data_val_lanes_ptr[3]), *((unsigned char *) data_val_lanes_ptr[2]), *((unsigned char *) data_val_lanes_ptr[1]), *((unsigned char *) data_val_lanes_ptr[0]));
		data_val_lanes_ptr = _mm256_add_epi64(data_val_lanes_ptr, _mm256_set1_epi64x(1ULL));
		...
		// diff.u = _mm256_set_epi64x(*((uint64_t *) (data_val_lanes[3]+1)), *((uint64_t *) (data_val_lanes[2]+1)), *((uint64_t *) (data_val_lanes[1]+1)), *((uint64_t *) (data_val_lanes[0]+1)));
		diff.u = _mm256_set_epi64x(*((uint64_t *) data_val_lanes_ptr[3]), *((uint64_t *) data_val_lanes_ptr[2]), *((uint64_t *) data_val_lanes_ptr[1]), *((uint64_t *) data_val_lanes_ptr[0]));
		// diff.u = _mm256_set_epi64x(*((uint64_t *) (data_val_lanes_ptr[3]+1)), *((uint64_t *) (data_val_lanes_ptr[2]+1)), *((uint64_t *) (data_val_lanes_ptr[1]+1)), *((uint64_t *) (data_val_lanes_ptr[0]+1)));

		// flag = _mm256_cmpgt_epi64(counter, _mm256_set1_epi64x(0ULL));

		// data_val_lanes_ptr = data_val_lanes_ptr + len;
		data_val_lanes_ptr = _mm256_add_epi64(data_val_lanes_ptr, len);
