...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	while (j<num_vals)
	{
		...
		for (;j<num_vals;j++)
		{
			if (window[rev_permutation_buf_2[j]] != window[rev_permutation_buf_2[j-1]])
				break;
			rf++;
		}
		if (rf != window_rf[rev_permutation_buf_2[k]])
		{
			if (tnum == 0)
			{
				for (i=0;i<num_vals;i++)
				{
					printf("%2ld: % 30.20g %2d        %2d: % 30.20g %2d\n", i, window[i], window_rf[i], rev_permutation_buf_2[i], window[rev_permutation_buf_2[i]], window_rf[rev_permutation_buf_2[i]]);
				}
				printf("\n");
			}
			error("wrong bucketsort");
		}
		k = j;
	}
	// TEST ======================================================================================================================================
	...
	for (l=0;l<num_rfs;l++)
	{
		...
		for (i=0;i<num_vals_div;i+=4)
		{
			k = k_s + i;
			// lane_id = i % 4;
			for (j=0;j<rf;j++)
			{
				vals_buf[rev_permutation[l_s + 4*j + 0]] = window[k + 0];
				vals_buf[rev_permutation[l_s + 4*j + 1]] = window[k + 1];
				vals_buf[rev_permutation[l_s + 4*j + 2]] = window[k + 2];
				vals_buf[rev_permutation[l_s + 4*j + 3]] = window[k + 3];
			}
		}

		l_s += rf * num_elems_unique;
		...
		for (l=0;l<num_rfs;l++)
		{
			...
			for (i=0;i<num_elems_unique;i++)
			{
				...
				if ((tnum == 0) && (num_rfs > 1))
				{
					for (j=0;j<rf;j++)
					{
						printf("lane=%ld rf=%ld num_elems_unique=%ld rev_perm=%d val=%g window=%g\n", lane_id, rf, num_elems_unique, rev_permutation[l_s + j], vals[rev_permutation[l_s + j]], window[k]);
					}
				}
				l_s += rf;
			}
	...
	// test_permutation(num_vals, vals, window, rev_permutation, "interleave");
	...
	for (k=0;k<num_vals;k++)
	{
		// if (window[k] != vals[rev_permutation[k]])
			// error("wrong");
