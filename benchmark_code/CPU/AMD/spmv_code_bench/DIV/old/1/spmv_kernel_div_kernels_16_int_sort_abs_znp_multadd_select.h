...
gather_coords_dense(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out)
{
	...
	coords.v = _mm256_set_epi64x(*((uint64_t *) &data_coords[(i+3)*coords_bytes]), *((uint64_t *) &data_coords[(i+2)*coords_bytes]), *((uint64_t *) &data_coords[(i+1)*coords_bytes]), *((uint64_t *) &data_coords[i*coords_bytes]));
	// *row_rel_out = coords.v & row_bits_mask;
	*row_rel_out = _mm256_and_si256(coords.v, row_bits_mask);
	*col_rel_out = _mm256_set_epi64x(bits_u64_extract(coords.u[3], row_bits, col_bits), bits_u64_extract(coords.u[2], row_bits, col_bits), bits_u64_extract(coords.u[1], row_bits, col_bits), bits_u64_extract(coords.u[0], row_bits, col_bits));
}
...
static __attribute__((always_inline)) inline
void
mult_add_serial(ValueType * x_rel, ValueType * y_rel, __m256d val, __m256i row_rel_in, __m256i col_rel_in)
{
	union {
		__m256i v;
		uint64_t u[4];
...
static __attribute__((always_inline)) inline
void
mult_add_r0(__m256d * v_res_ptr, ValueType * x_rel, __m256d val, __m256i col_rel_in)
{
	__m256d v_x;
	union {
		__m256i v;
...
static __attribute__((always_inline)) inline
void
mult_add_ybuf(long num_rows, double * y_buf_in, ValueType * x_rel, __m256d val, __m256i row_rel_in, __m256i col_rel_in)
{
	__m256d v_x, v_y;
...
decompress_and_compute_kernel_sort_diff_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate)
{
	...
	if (row_bits == 0)
	{
		return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0, 1);
		// switch (col_bits) {
			// case 8:
