...
static unsigned char *** t_data_lens;
...
compress_init_sort_diff(__attribute__((unused)) ValueType * vals, __attribute__((unused)) const long num_vals, long packet_size)
{
	...
	t_data_lens = (typeof(t_data_lens)) malloc(num_threads * sizeof(*t_data_lens));
	...
	#pragma omp parallel
	{
		...
		t_data_lens[tnum] = (typeof(&(**t_data_lens))) malloc(4 * sizeof(**t_data_lens));
		for (i=0;i<4;i++)
			t_data_lens[tnum][i] = (typeof(&(**t_data_lens[i]))) malloc(8*window_size * sizeof(**t_data_lens[i]));
...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals_unpadded, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	unsigned char ** data_val_lens = t_data_lens[tnum];
	...
	struct Byte_Stream Bs_lens[4];
	struct Byte_Stream Bs_vals[4];
	...
	*((double *) &data_intro[data_intro_bytes]) = window[3];
	...
	data_intro_bytes += 4 * sizeof(*data_val_lens_size);
	...
	bytestream_init_write(&Bs_lens[0], data_val_lens[0]);
	bytestream_init_write(&Bs_lens[1], data_val_lens[1]);
	bytestream_init_write(&Bs_lens[2], data_val_lens[2]);
	bytestream_init_write(&Bs_lens[3], data_val_lens[3]);

	bytestream_init_write(&Bs_vals[0], data_val_lanes[0]);
	bytestream_init_write(&Bs_vals[1], data_val_lanes[1]);
	bytestream_init_write(&Bs_vals[2], data_val_lanes[2]);
	bytestream_init_write(&Bs_vals[3], data_val_lanes[3]);
	...
	for (i=4;i<num_vals_padded;i++)
	{
		...
		{
			len = 0;
			bytestream_write_unsafe_cast(&Bs_lens[i%4], len, 1);
		}
		else
		{
			...
			uint64_t len_buf = len;
			len |=  (trailing_zero_bits_div4 << 4ULL);
			bytestream_write_unsafe_cast(&Bs_lens[i%4], len, 1);
			bytestream_write_unsafe_cast(&Bs_vals[i%4], diff.u, len_buf);
	...
	data_val_lens_size[0] = Bs_lens[0].len;
	data_val_lens_size[1] = Bs_lens[1].len;
	data_val_lens_size[2] = Bs_lens[2].len;
	data_val_lens_size[3] = Bs_lens[3].len;

	data_val_lanes_size[0] = Bs_vals[0].len;
	data_val_lanes_size[1] = Bs_vals[1].len;
	data_val_lanes_size[2] = Bs_vals[2].len;
	data_val_lanes_size[3] = Bs_vals[3].len;

	unsigned char * ptr = &data_coords[data_coords_bytes];

	memcpy(ptr, data_val_lens[0], Bs_lens[0].len);
	ptr += Bs_lens[0].len;
	memcpy(ptr, data_val_lens[1], Bs_lens[1].len);
	ptr += Bs_lens[1].len;
	memcpy(ptr, data_val_lens[2], Bs_lens[2].len);
	ptr += Bs_lens[2].len;
	memcpy(ptr, data_val_lens[3], Bs_lens[3].len);
	ptr += Bs_lens[3].len;
	const uint64_t data_val_lens_bytes = Bs_lens[0].len + Bs_lens[1].len + Bs_lens[2].len + Bs_lens[3].len;

	memcpy(ptr, data_val_lanes[0], Bs_vals[0].len);
	ptr += Bs_vals[0].len;
	memcpy(ptr, data_val_lanes[1], Bs_vals[1].len);
	ptr += Bs_vals[1].len;
	memcpy(ptr, data_val_lanes[2], Bs_vals[2].len);
	ptr += Bs_vals[2].len;
	memcpy(ptr, data_val_lanes[3], Bs_vals[3].len);
	ptr += Bs_vals[3].len;
	const uint64_t data_val_lanes_bytes = Bs_vals[0].len + Bs_vals[1].len + Bs_vals[2].len + Bs_vals[3].len;
		...
		)
{
	...
	__m256i row_rel, col_rel;
	__m256i len;
	__m256i data_val_lanes_ptr;
	__m256i flag, counter;
	long num_vals;
	...
	__m256i data_val_lens;
	data_val_lens[0] = (long long) &data_coords[data_coords_bytes];
	data_val_lens[1] = data_val_lens[0] + data_val_lens_size[0];
	data_val_lens[2] = data_val_lens[1] + data_val_lens_size[1];
	data_val_lens[3] = data_val_lens[2] + data_val_lens_size[2];
	const uint64_t data_val_lens_bytes = data_val_lens_size[0] + data_val_lens_size[1] + data_val_lens_size[2] + data_val_lens_size[3];

	__m256i data_val_lanes;
	data_val_lanes[0] = data_val_lens[0] + data_val_lens_bytes;
	...
	__m256i data_val_lens_i = _mm256_set1_epi64x(0ULL);
	counter = _mm256_set1_epi64x(0ULL);
	for (i=4;i<num_vals_padded;i+=4)
	{
		gather_coords(i, data_coords, coords_bytes, row_bits, col_bits, &row_rel, &col_rel);

		// len = _mm256_set_epi64x(((unsigned char *) data_val_lens[3])[i/4-1], ((unsigned char *) data_val_lens[2])[i/4-1], ((unsigned char *) data_val_lens[1])[i/4-1], ((unsigned char *) data_val_lens[0])[i/4-1]);
		// len = _mm256_set_epi64x(*((unsigned char *) data_val_lens[3]), *((unsigned char *) data_val_lens[2]), *((unsigned char *) data_val_lens[1]), *((unsigned char *) data_val_lens[0]));
		// data_val_lens = _mm256_add_epi64(data_val_lens, _mm256_set1_epi64x(1ULL));
		// len = _mm256_set_epi64x(((unsigned char *) data_val_lens[3])[data_val_lens_i[3]], ((unsigned char *) data_val_lens[2])[data_val_lens_i[2]], ((unsigned char *) data_val_lens[1])[data_val_lens_i[1]], ((unsigned char *) data_val_lens[0])[data_val_lens_i[0]]);
		// data_val_lens_i = _mm256_add_epi64(data_val_lens_i, _mm256_set1_epi64x(1ULL));
		// len[0] = ((unsigned char *) data_val_lens[0])[data_val_lens_i[0]];
		// len[1] = ((unsigned char *) data_val_lens[1])[data_val_lens_i[1]];
		// len[2] = ((unsigned char *) data_val_lens[2])[data_val_lens_i[2]];
		// len[3] = ((unsigned char *) data_val_lens[3])[data_val_lens_i[3]];
		// data_val_lens_i = _mm256_add_epi64(data_val_lens_i, _mm256_set1_epi64x(1ULL));
		len[0] = *((unsigned char *) data_val_lens[0]);
		len[1] = *((unsigned char *) data_val_lens[1]);
		len[2] = *((unsigned char *) data_val_lens[2]);
		len[3] = *((unsigned char *) data_val_lens[3]);
		data_val_lens = _mm256_add_epi64(data_val_lens, _mm256_set1_epi64x(1ULL));
		...
		// flag = _mm256_cmpgt_epi64(counter, _mm256_set1_epi64x(0ULL));
