...
static unsigned char ** t_data_vals;
...
compress_init_sort_diff(__attribute__((unused)) ValueType * vals, __attribute__((unused)) const long num_vals, long packet_size)
{
	...
	t_data_vals = (typeof(t_data_vals)) malloc(num_threads * sizeof(*t_data_vals));
	...
	#pragma omp parallel
	{
		...
		long i;
		t_vals[tnum] = (typeof(&(**t_vals))) malloc(window_size * sizeof(**t_vals));   // Can't set to void * without cast, also sees 't_vals[tnum]' as 'double*&' so we need to take the address of **. C++ is so nice, isn't it?
		...
		t_data_vals[tnum] = (typeof(&(**t_data_vals))) malloc(8*window_size * sizeof(**t_data_vals));
...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals_unpadded, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	unsigned char * data_vals = t_data_vals[tnum];
	int * data_vals_bytes;
	...
	*((double *) &data_intro[data_intro_bytes]) = window[3];
	...
	data_vals_bytes = (typeof(data_vals_bytes)) &(data_intro[data_intro_bytes]);
	data_intro_bytes += sizeof(*data_vals_bytes);
	...
	bytestream_init_write(&Bs, data_vals);
	...
	for (i=4;i<num_vals_padded;i++)
	{
		...
		else
		{
			...
			bytestream_write_unsafe_cast(&Bs, diff.u, len);
	...
	*data_vals_bytes = Bs.len;

	unsigned char * ptr = &data_val_lens[data_val_lens_bytes];
	memcpy(ptr, data_vals, Bs.len);

	if (num_vals_out != NULL)
		*num_vals_out = num_vals;

	return data_intro_bytes + data_coords_bytes + data_val_lens_bytes + *data_vals_bytes;
}
		...
		)
{
	...
	unsigned char * data_vals;
	data_vals = (typeof(data_vals)) &data_val_lens[data_val_lens_bytes];
	...
	for (i=4;i<num_vals_padded;i+=4)
	{
		...
		diff.v[0] = *((uint64_t *) data_vals);
		data_vals += len[0];
		diff.v[1] = *((uint64_t *) data_vals);
		data_vals += len[1];
		diff.v[2] = *((uint64_t *) data_vals);
		data_vals += len[2];
		diff.v[3] = *((uint64_t *) data_vals);
		data_vals += len[3];

		// data_vals = data_vals + len;
		// data_vals = _mm256_add_epi64(data_vals, len);
