...
compress_kernel_sort_diff(INT_T * row_ptr, INT_T * ja, ValueType * vals, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	struct Byte_Stream Bs[BLOCK_SIZE];
	...
	long num_vals_mod = num_vals % BLOCK_SIZE;
	long num_vals_div = num_vals / BLOCK_SIZE;
	...
	for (k=0;k<num_vals;k++)
	{
		l = (k % BLOCK_SIZE) * num_vals_div + k / BLOCK_SIZE;   // interleave index
		l += (k % BLOCK_SIZE < num_vals_mod) ? k % BLOCK_SIZE : num_vals_mod;
	...
	*((uint32_t *) &data_intro[data_intro_bytes]) = col_min;
	...
	data_intro_bytes += BLOCK_SIZE * sizeof(*data_val_lanes_size);
	...
	for (long iter=0;iter<BLOCK_SIZE;iter++)
		bytestream_init_write(&Bs[iter], data_val_lanes[iter]);

	union {
		double d;
		uint64_t u;
	} val_prev[BLOCK_SIZE], val;
	for (long iter=0;iter<BLOCK_SIZE;iter++)
		val_prev[iter].d = 0;
	for (i=0;i<num_vals;i++)
	{
		lane_id = i % BLOCK_SIZE;
		val.d = window[i];
...
{
	const int tidg = cuda_get_thread_num();
	const int tidb = threadIdx.x;
	const int block_id = blockIdx.x;
	int tid_b = omp_get_thread_num();
	ValueType * x_rel;
	ValueType * y_rel;
	double * window = t_window[block_id];
	long num_vals;
	...
	long num_vals_div = num_vals / BLOCK_SIZE;
	long num_vals_mul4 = num_vals_div * BLOCK_SIZE;
	...
	int * data_val_lanes_sizes = (typeof(data_val_lanes_size)) &(data_intro[data_intro_bytes]);
	int data_val_lanes_size = data_val_lanes_sizes[tidb];
	data_intro_bytes += BLOCK_SIZE * sizeof(*data_val_lanes_size);
	...
	for (long iter=1;iter<BLOCK_SIZE;iter++)
	{
	...
	for (i=0;i<num_vals_mul4;i+=BLOCK_SIZE)
	{
		union {
			vec_d_t d;
			vec_i_t u;
