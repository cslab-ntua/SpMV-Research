...
void
gather_coords_dense(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	const __m128i row_bits_mask = _mm_set1_epi32((1ULL<<row_bits) - 1);   // here '-' has precedence over '<<'!
	union {
		__m128i v;
		uint32_t u[4];
	} coords;
	coords.v = _mm_set_epi32(*((uint32_t *) &data_coords[(i+3)*coords_bytes]), *((uint32_t *) &data_coords[(i+2)*coords_bytes]), *((uint32_t *) &data_coords[(i+1)*coords_bytes]), *((uint32_t *) &data_coords[i*coords_bytes]));
	// *row_rel_out = coords.v & row_bits_mask;
	*row_rel_out = _mm_and_si128(coords.v, row_bits_mask);
	*col_rel_out = _mm_set_epi32(bits_u32_extract(coords.u[3], row_bits, col_bits), bits_u32_extract(coords.u[2], row_bits, col_bits), bits_u32_extract(coords.u[1], row_bits, col_bits), bits_u32_extract(coords.u[0], row_bits, col_bits));
}

static __attribute__((always_inline)) inline
void
gather_coords_sparse_r0(long i, unsigned char * data_coords, const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 0");
	const __m128i col_bits_mask = _mm_set1_epi32((1ULL<<col_bits) - 1);
	__m128i col_rel;
	col_rel = _mm_set_epi32(data_coords[(i+3)*coords_bytes], data_coords[(i+2)*coords_bytes], data_coords[(i+1)*coords_bytes], data_coords[i*coords_bytes]);
	// col_rel = col_rel & col_bits_mask;
	col_rel = _mm_and_si128(col_rel, col_bits_mask);
	*row_rel_out = _mm_set1_epi32(0);
	*col_rel_out = col_rel;
}

static __attribute__((always_inline)) inline
void
gather_coords_sparse_r0_c1(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 0,1");
	__m128i col_rel;
	col_rel = _mm_set_epi32(data_coords[(i+3)*1], data_coords[(i+2)*1], data_coords[(i+1)*1], data_coords[i*1]);
	*row_rel_out = _mm_set1_epi32(0);
	*col_rel_out = col_rel;
}
...
static __attribute__((always_inline)) inline
void
gather_coords_sparse_r0_c2(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 0,2");
	__m128i col_rel;
	col_rel = _mm_set_epi32(*((uint16_t *) &data_coords[(i+3)*2]), *((uint16_t *) &data_coords[(i+2)*2]), *((uint16_t *) &data_coords[(i+1)*2]), *((uint16_t *) &data_coords[i*2]));
	*row_rel_out = _mm_set1_epi32(0);
	*col_rel_out = col_rel;
}
...
static __attribute__((always_inline)) inline
void
gather_coords_sparse_r0_c3(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 0,3");
	const __m128i col_bits_mask = _mm_set1_epi32((1ULL<<24) - 1);
	__m128i col_rel;
	col_rel = _mm_set_epi32(data_coords[(i+3)*3], data_coords[(i+2)*3], data_coords[(i+1)*3], data_coords[i*3]);
	// col_rel = col_rel & col_bits_mask;
	col_rel = _mm_and_si128(col_rel, col_bits_mask);
	*row_rel_out = _mm_set1_epi32(0);
	*col_rel_out = col_rel;
...
static __attribute__((always_inline)) inline
void
gather_coords_sparse_r0_c4(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 0,4");
	__m128i col_rel;
	col_rel = _mm_set_epi32(*((uint32_t *) &data_coords[(i+3)*4]), *((uint32_t *) &data_coords[(i+2)*4]), *((uint32_t *) &data_coords[(i+1)*4]), *((uint32_t *) &data_coords[i*4]));
	*row_rel_out = _mm_set1_epi32(0);
	*col_rel_out = col_rel;
}
...
/* static __attribute__((always_inline)) inline
void
gather_coords_sparse_r1(long i, unsigned char * data_coords, const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	const __m128i col_bits_mask = _mm_set1_epi32((1ULL<<col_bits) - 1);
	__m128i row_rel, col_rel;
	row_rel = _mm_set_epi32(data_coords[(i+3)*coords_bytes], data_coords[(i+2)*coords_bytes], data_coords[(i+1)*coords_bytes], data_coords[i*coords_bytes]);
	col_rel = _mm_set_epi32(*((uint32_t *) &data_coords[(i+3)*coords_bytes + 1]), *((uint32_t *) &data_coords[(i+2)*coords_bytes + 1]), *((uint32_t *) &data_coords[(i+1)*coords_bytes + 1]), *((uint32_t *) &data_coords[i*coords_bytes + 1]));
	// col_rel = col_rel & col_bits_mask;
	col_rel = _mm_and_si128(col_rel, col_bits_mask);
	*row_rel_out = row_rel;
	*col_rel_out = col_rel;
...
static __attribute__((always_inline)) inline
void
gather_coords_sparse_r1_c1(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 1\n");
	#if 1
	__m128i row_rel, col_rel;
	row_rel = _mm_set_epi32(data_coords[(i+3)*2], data_coords[(i+2)*2], data_coords[(i+1)*2], data_coords[i*2]);
	col_rel = _mm_set_epi32(data_coords[(i+3)*2 + 1], data_coords[(i+2)*2 + 1], data_coords[(i+1)*2 + 1], data_coords[i*2 + 1]);
	*row_rel_out = row_rel;
	*col_rel_out = col_rel;
	#else
	__m128i v = _mm_loadu_si64(&data_coords[i*2]);
	*row_rel_out = _mm_shuffle_epi8(v, _mm_set_epi32(0xFFFFFF06UL, 0xFFFFFF04UL, 0xFFFFFF02UL, 0xFFFFFF00UL));
	*col_rel_out = _mm_shuffle_epi8(v, _mm_set_epi32(0xFFFFFF07UL, 0xFFFFFF05UL, 0xFFFFFF03UL, 0xFFFFFF01UL));
	// uint64_t u = *((uint64_t *) &data_coords[i*2]);
	// __m256i v = _mm256_set_epi64x(u, u, u, u);
	// v = _mm256_shuffle_epi8(v, _mm256_set_epi32(0xFFFFFF07UL, 0xFFFFFF05UL, 0xFFFFFF03UL, 0xFFFFFF01UL, 0xFFFFFF06UL, 0xFFFFFF04UL, 0xFFFFFF02UL, 0xFFFFFF00UL));
	// *row_rel_out = _mm256_castsi256_si128(v);
	// *col_rel_out = _mm256_extracti128_si256(v, 1);
	#endif
}

static __attribute__((always_inline)) inline
void
gather_coords_sparse_r1_c2(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 2");
	#if 1
	__m128i row_rel, col_rel;
	row_rel = _mm_set_epi32(data_coords[(i+3)*3], data_coords[(i+2)*3], data_coords[(i+1)*3], data_coords[i*3]);
	col_rel = _mm_set_epi32(*((uint16_t *) &data_coords[(i+3)*3 + 1]), *((uint16_t *) &data_coords[(i+2)*3 + 1]), *((uint16_t *) &data_coords[(i+1)*3 + 1]), *((uint16_t *) &data_coords[i*3 + 1]));
	*row_rel_out = row_rel;
	*col_rel_out = col_rel;
	#else
	__m128i v = _mm_loadu_si128((__m128i *) &data_coords[i*3]);
	*row_rel_out = _mm_shuffle_epi8(v, _mm_set_epi32(0xFFFFFF09UL, 0xFFFFFF06UL, 0xFFFFFF03UL, 0xFFFFFF00UL));
	*col_rel_out = _mm_shuffle_epi8(v, _mm_set_epi32(0xFFFF0B0AUL, 0xFFFF0807UL, 0xFFFF0504UL, 0xFFFF0201UL));
	// __m128i v128 = _mm_loadu_si128((__m128i *) &data_coords[i*3]);
	// __m256i v = _mm256_set_m128i(v128, v128);
	// v = _mm256_shuffle_epi8(v, _mm256_set_epi32(0xFFFF0B0AUL, 0xFFFF0807UL, 0xFFFF0504UL, 0xFFFF0201UL, 0xFFFFFF09UL, 0xFFFFFF06UL, 0xFFFFFF03UL, 0xFFFFFF00UL));
	// *row_rel_out = _mm256_castsi256_si128(v);
	// *col_rel_out = _mm256_extracti128_si256(v, 1);
	#endif
}
...
static __attribute__((always_inline)) inline
void
gather_coords_sparse_r1_c3(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 3");
	#if 1
	const __m128i col_bits_mask = _mm_set1_epi32((1ULL<<24) - 1);
	__m128i row_rel, col_rel;
	row_rel = _mm_set_epi32(data_coords[(i+3)*4], data_coords[(i+2)*4], data_coords[(i+1)*4], data_coords[i*4]);
	col_rel = _mm_set_epi32(*((uint32_t *) &data_coords[(i+3)*4 + 1]), *((uint32_t *) &data_coords[(i+2)*4 + 1]), *((uint32_t *) &data_coords[(i+1)*4 + 1]), *((uint32_t *) &data_coords[i*4 + 1]));
	// col_rel = col_rel & col_bits_mask;
	col_rel = _mm_and_si128(col_rel, col_bits_mask);
	*row_rel_out = row_rel;
	*col_rel_out = col_rel;
	#else
	// __m128i v = _mm_loadu_si128((__m128i *) &data_coords[i*4]);
	// *row_rel_out = _mm_shuffle_epi8(v, _mm_set_epi32(0xFFFFFF0CUL, 0xFFFFFF08UL, 0xFFFFFF04UL, 0xFFFFFF00UL));
	// *col_rel_out = _mm_shuffle_epi8(v, _mm_set_epi32(0xFF0F0E0DUL, 0xFF0B0A09UL, 0xFF070605UL, 0xFF030201UL));
	__m256i v = _mm256_broadcastsi128_si256(_mm_loadu_si128((__m128i *) &data_coords[i*3]));
	v = _mm256_shuffle_epi8(v, _mm256_set_epi32(0xFF0F0E0DUL, 0xFF0B0A09UL, 0xFF070605UL, 0xFF030201UL, 0xFFFFFF0CUL, 0xFFFFFF08UL, 0xFFFFFF04UL, 0xFFFFFF00UL));
	*row_rel_out = _mm256_castsi256_si128(v);
	*col_rel_out = _mm256_extracti128_si256(v, 1);
	#endif
}
...
static __attribute__((always_inline)) inline
void
gather_coords_sparse_r1_c4(long i, unsigned char * data_coords, __attribute__((unused)) const uint64_t coords_bytes, __attribute__((unused)) uint64_t row_bits, __attribute__((unused)) uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out)
{
	// error("test 4");
	__m128i row_rel, col_rel;
	row_rel = _mm_set_epi32(data_coords[(i+3)*5], data_coords[(i+2)*5], data_coords[(i+1)*5], data_coords[i*5]);
	col_rel = _mm_set_epi32(*((uint32_t *) &data_coords[(i+3)*5 + 1]), *((uint32_t *) &data_coords[(i+2)*5 + 1]), *((uint32_t *) &data_coords[(i+1)*5 + 1]), *((uint32_t *) &data_coords[i*5 + 1]));
	*row_rel_out = row_rel;
	*col_rel_out = col_rel;
}
...
static __attribute__((always_inline)) inline
void
mult_add_serial(ValueType * x_rel, ValueType * y_rel, __m256d val, __m128i row_rel_in, __m128i col_rel_in)
{
	union {
		__m128i v;
		uint32_t u[4];
	} row_rel, col_rel;
	row_rel.v = row_rel_in;
	col_rel.v = col_rel_in;
	y_rel[row_rel.u[0]] += val[0] * x_rel[col_rel.u[0]];
	y_rel[row_rel.u[1]] += val[1] * x_rel[col_rel.u[1]];
	y_rel[row_rel.u[2]] += val[2] * x_rel[col_rel.u[2]];
	y_rel[row_rel.u[3]] += val[3] * x_rel[col_rel.u[3]];
}


static __attribute__((always_inline)) inline
void
mult_add_r0(__m256d * v_res_ptr, ValueType * x_rel, __m256d val, __m128i col_rel_in)
{
	__m256d v_x;
	union {
		__m128i v;
	...
	col_rel.v = col_rel_in;
	v_x = _mm256_set_pd(x_rel[col_rel.u[3]], x_rel[col_rel.u[2]], x_rel[col_rel.u[1]], x_rel[col_rel.u[0]]);
	*v_res_ptr = _mm256_fmadd_pd(val, v_x, *v_res_ptr);
}
...
{
	*y_rel += hsum256_pd(v_res);
}


static __attribute__((always_inline)) inline
void
mult_add_ybuf(long num_rows, double * y_buf_in, ValueType * x_rel, __m256d val, __m128i row_rel_in, __m128i col_rel_in)
{
	__m256d v_x, v_y;
	...
	col_rel.v = col_rel_in;
	v_x = _mm256_set_pd(x_rel[col_rel.u[3]], x_rel[col_rel.u[2]], x_rel[col_rel.u[1]], x_rel[col_rel.u[0]]);
	v_y = _mm256_set_pd(y_buf[3][row_rel.u[3]], y_buf[2][row_rel.u[2]], y_buf[1][row_rel.u[1]], y_buf[0][row_rel.u[0]]);
	v_y = _mm256_fmadd_pd(val, v_x, v_y);
	y_buf[0][row_rel.u[0]] = v_y[0];
	y_buf[1][row_rel.u[1]] = v_y[1];
	y_buf[2][row_rel.u[2]] = v_y[2];
	y_buf[3][row_rel.u[3]] = v_y[3];
}
...
decompress_and_compute_kernel_sort_diff_base(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate,
		void (* gather_coords)(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, __m128i * row_rel_out, __m128i * col_rel_out),
		int mult_add_select
		)
{
	int tnum = omp_get_thread_num();
	...
	i = 0;
	gather_coords(i, data_coords, coords_bytes, row_bits, col_bits, &row_rel.v, &col_rel.v);
	...
	else
	{
		switch (mult_add_select) {
			case 0: mult_add_serial(x_rel, y_rel, val.d, row_rel.v, col_rel.v); break;
			case 1: mult_add_r0(&mult_add_r0_res, x_rel, val.d, col_rel.v); break;
			case 2: mult_add_ybuf(num_rows, y_buf, x_rel, val.d, row_rel.v, col_rel.v); break;
		}
	}
	...
	for (i=4;i<num_vals_padded;i+=4)
	{
		...
		{
			// v_x = _mm256_set_pd(x_rel[col_rel.u[3]], x_rel[col_rel.u[2]], x_rel[col_rel.u[1]], x_rel[col_rel.u[0]]);
			// v_x = _mm256_i64gather_pd(x_rel, col_rel.v, 8);
		}
	...
	{
		switch (mult_add_select) {
			case 1: mult_add_r0_finalize(mult_add_r0_res, y_rel); break;
...
decompress_and_compute_kernel_sort_diff_select(unsigned char * restrict buf, ValueType * restrict x, ValueType * restrict y, long * restrict num_vals_out, const int validate)
{
	...
	if (row_bits == 0)
	{
		switch (col_bits) {
			case 8:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c1, 1);
			case 16:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c2, 1);
			case 24:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c3, 1);
			case 32:
				return decompress_and_compute_kernel_sort_diff_base(buf, x, y, num_vals_out, validate, gather_coords_sparse_r0_c4, 1);
		}
	}
