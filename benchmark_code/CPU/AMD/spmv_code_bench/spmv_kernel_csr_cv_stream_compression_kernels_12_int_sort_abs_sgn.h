...
quicksort_cmp(int a, int b, double * vals)
{
	return (fabs(vals[a]) > fabs(vals[b])) ? 1 : (fabs(vals[a]) < fabs(vals[b])) ? -1 : 0;
}
...
compress_kernel_sort_diff(INT_T * ia, INT_T * ja, ValueType * vals_unpadded, long i_s, long j_s, unsigned char * buf, long num_vals, long * num_vals_out)
{
	...
	long force_row_index_lte_1_byte = 0;
	...
	*((double *) &data_intro[data_intro_bytes]) = window[3];
	...
	for (i=4;i<num_vals_padded;i++)
	{
		...
		// diff = compress_diff_lossy(window[i], val_prev[i%4].d, diff, 1.0e-9);

		// val_prev[i%4].u += diff;
		val_prev[i%4].d = window[i];
		if (diff.u == 0)
		{
			len = 0;
			data_val_lens[i] = 0;
		...
		else
		{
			 ...
			 */
			uint64_t mask_sgn = 1ULL << 63;
			uint64_t sgn = diff.u & mask_sgn;
			uint64_t tmp = diff.u;
			diff.u &= ~mask_sgn;
			leading_zeros = __builtin_clzl(diff.u);
			leading_zero_bytes = leading_zeros >> 3;
			len = 8 - leading_zero_bytes;
			if (len > 8)
				error("bad len");
		...
		void (* gather_coords)(long i, unsigned char * data_coords, const uint64_t coords_bytes, uint64_t row_bits, uint64_t col_bits, __m256i * row_rel_out, __m256i * col_rel_out))
{
	...
	// row_bits_mask = _mm256_set1_epi64x((1ULL<<row_bits) - 1);   // here '-' has precedence over '<<'!
	// col_bits_mask = _mm256_set1_epi64x((1ULL<<col_bits) - 1);
	...
	for (i=4;i<num_vals_padded;i+=4)
	{
		...
		const __m256i mask_sgn = _mm256_set1_epi64x(1ULL << 7);
		__m256i sgn = len & mask_sgn;
		len &= ~mask_sgn;
		sgn <<= 56;
