...
def gen_vectors(spm, isPcg, mtxName, vecPath, parEntries):
    ...
    print("\tGEN_VECTORS - write_vec/refVec\t",round(end - start,3))
...
def process_matrices(isGenVec, isPcg, isClean, mtxList, parEntries, vecPath):
    ...
    for line in downloadNames:
        ...
        mtxName = mtxFileName.strip(".mtx") # mtxName = strList[0]
        ...
        if not path.exists(mtxVecPath):
            ...
            if isGenVec:
                ...
                print("\tGEN_VECTORS - read_matrix\t",round(end - start,3))
...
def main(args):
    if (args.usage):
        print('Usage example:')
        print('python3 gen_vectors.py --gen_vec [--pcg] [--clean] --mtx_list ./test_matrices.txt --vec_path ./vec_dat')
    else:
