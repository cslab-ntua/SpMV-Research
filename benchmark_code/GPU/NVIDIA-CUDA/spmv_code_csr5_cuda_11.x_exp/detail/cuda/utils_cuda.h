...
double __shfl_down_sync(double var, unsigned int srcLane, int width=32)
{
...
__device__ __forceinline__
double __shfl_up_sync(double var, unsigned int srcLane, int width=32)
{
    int2 a = *reinterpret_cast<int2*>(&var);
    a.x = __shfl_up_sync(a.x, srcLane, width);
    a.y = __shfl_up_sync(a.y, srcLane, width);
    return *reinterpret_cast<double*>(&a);
}

__device__ __forceinline__
double __shfl_xor_sync(double var, int srcLane, int width=32)
{
    int2 a = *reinterpret_cast<int2*>(&var);
...
// FIXME - PETROS: This is an updated version for step...who knows what happens here
enum
{
