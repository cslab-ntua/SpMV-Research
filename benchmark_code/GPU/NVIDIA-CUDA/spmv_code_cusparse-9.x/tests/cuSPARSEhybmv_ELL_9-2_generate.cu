...
int main(int argc, char **argv) {
	/// Check Input
	massert(argc == 13,
		  "Incorrect arguments.\nUsage:  ./Executable logfilename Mpakos_11_parameters_with_spaces");
	...
	cuSPARSE_op.format_convert(SPMV_FORMAT_ELL);
	...
	fprintf(stdout,"Testing cuSPARSE_ell...\t");

	cuSPARSE_op.cuSPARSE_ell();
	...
	// Warmup
	for (int i = 0; i < 100; i++) cuSPARSE_op.cuSPARSE_ell();
	cudaDeviceSynchronize();

	// Run cuSPARSE csr
	fprintf(stdout,"Timing cuSPARSE_ell...\n");
	...
	sprintf(powa_filename, "cuSPARSE_ellmv_9-2_generate_cudatype-%d_format-CSR.log", CUDA_VALUE_TYPE_AX);
	...
	NvemStats_p nvem_data = NvemStopMeasure(device_id, "Energy measure cuSPARSE_ellmv_9-2_generate");
	gflops_s = op.flops*1e-9/op.timer;
	double W_avg = nvem_data->W_avg, J_estimated = nvem_data->J_estimated/(NR_ITER+extra_itter); 
	fprintf(stdout, "cuSPARSE_ell9-2: t = %lf ms (%lf Gflops/s ). Average Watts = %lf, Estimated Joules = %lf\n", op.timer*1000, gflops_s, W_avg, J_estimated);
	...
	"," << op.skew << "," << op.avg_num_neighbours << "," << op.cross_row_similarity <<
	"," << "cuSPARSE_ell9-2" <<  "," << op.timer << "," << gflops_s << "," << W_avg <<  "," << J_estimated << endl;
