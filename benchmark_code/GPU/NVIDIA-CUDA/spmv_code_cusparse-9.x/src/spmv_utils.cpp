...
void spmv_csr(int *csrPtr, int *csrCol, VALUETYPE *csrVal, VALUETYPE *x, VALUETYPE *ys,
              int m) {
  ddebug(" -> spmv_csr()\n");
  int i, j;
  for (i = 0; i < m; ++i) {
...
template void spmv_csr<double>(int *csrPtr, int *csrCol, double *csrVal, double *x, double *ys, int n);
template void spmv_csr<float>(int *csrPtr, int *csrCol, float *csrVal, float *x, float *ys, int n);
    ...
    case (SPMV_FORMAT_COO): {
      ...
      if (data->rowInd && data->colInd && data->values) {
        ...
        switch (value_type) {
          ...
          default:
            massert(false,
                    "SpmvOperator::spmv_data_copy_host -> Unsupported SpMV "
                    "value datatype");
        }
...
/*
  ...
  // TODO: might be a better socket allocation, but stick to merge one for now
  int val_socket;
  if (wrapper->sockets > 1)
    val_socket = 1;
  else
    val_socket = 0;
  ...
  switch (format) {
    case (SPMV_FORMAT_COO): {
      ...
      SpmvCooData *cp_data = (SpmvCooData *)malloc(sizeof(SpmvCooData));
      if (data->rowInd && data->colInd && data->values) {
        cp_data->rowInd = (int *)numalloc(nz * sizeof(int), 0, wrapper);
        cp_data->colInd = (int *)numalloc(nz * sizeof(int), 0, wrapper);
        ...
        switch (value_type) {
          case (SPMV_VALUETYPE_FLOAT):
            cp_data->values = numalloc(nz * sizeof(float), val_socket, wrapper);
            vec_copy<float>((float *)cp_data->values, (float *)data->values, nz,
                            0);
            break;
          case (SPMV_VALUETYPE_DOUBLE):
            cp_data->values =
                numalloc(nz * sizeof(double), val_socket, wrapper);
            vec_copy<double>((double *)cp_data->values, (double *)data->values,
                             nz, 0);
            break;
          case (SPMV_VALUETYPE_INT):
          ...
          default:
            massert(false,
                    "SpmvOperator::spmv_data_copy_numa -> Unsupported SpMV "
                    "value datatype");
        }

      } else
        debug(
            "SpmvOperator::spmv_data_copy_numa -> warning... empty Spmv "
            "struct, copied nothing\n");
      ddebug(" <- SpmvOperator::spmv_data_copy_numa()\n");
      return cp_data;
    }
    case (SPMV_FORMAT_CSR): {
      ...
      SpmvCsrData *cp_data = (SpmvCsrData *)malloc(sizeof(SpmvCsrData));
      if (data->rowPtr && data->colInd && data->values) {
        cp_data->rowPtr = (int *)numalloc((n + 1) * sizeof(int), 0, wrapper);
        cp_data->colInd = (int *)numalloc(nz * sizeof(int), 0, wrapper);

        vec_copy<int>(cp_data->rowPtr, data->rowPtr, n + 1, 0);
        vec_copy<int>(cp_data->colInd, data->colInd, nz, 0);
    ...
    case (SPMV_FORMAT_BSR): {
      ...
      // const int mb = (m + data->blockDim - 1) / data->blockDim;
      if (data->rowPtr && data->colInd && data->values) {
        cp_data->rowPtr = (int *)numalloc((nb + 1) * sizeof(int), 0, wrapper);
        cp_data->colInd = (int *)numalloc(data->nnzb * sizeof(int), 0, wrapper);
    ...
    case (SPMV_FORMAT_DIA):
      massert(0,
              "SpmvOperator::spmv_data_copy_numa -> SPMV_FORMAT_DIA not "
              "implemented");
    case (SPMV_FORMAT_ELL):
      massert(0,
              "SpmvOperator::spmv_data_copy_numa -> SPMV_FORMAT_ELL not "
              "implemented");
    case (SPMV_FORMAT_HYB):
      massert(0,
              "SpmvOperator::spmv_data_copy_numa -> SPMV_FORMAT_HYB not "
              "implemented");
    default:
      massert(0,
              "SpmvOperator::spmv_data_copy_numa -> Unreacheable format "
              "default reached");
      break;
  }
  ddebug(" <- SpmvOperator::spmv_data_copy_numa()\n");
