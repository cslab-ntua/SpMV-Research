...
for dtype in D #S
do
	for f in `cat ${MASTEDIR}/validation_matrices.in`;
	do
		fname=${f##*/}
		...
		do
			...
			do
				$exec $STORE_TIMER_DIR/silver1-TeslaV100_dtype-${dtype}_run_validation_matrices.csv $f  &>>$STORE_TIMER_DIR/silver1-TeslaV100_dtype-${dtype}_run_validation_matrices.err
			done
		done
	done
done
