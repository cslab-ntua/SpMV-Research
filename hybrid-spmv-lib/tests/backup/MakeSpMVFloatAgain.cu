...
int main(int argc, char **argv) {
  ...
  float *out = (float *)malloc(op.n * sizeof(float));
  vec_init_rand(x, op.m, 0);
  op.value_type = SPMV_VALUETYPE_FLOAT; 
  op.vec_alloc((float*)x);
  ...
  spmv_csr_f(data->rowPtr, data->colInd, (float *)data->values, (float *)op.x,
           (float *)op.y, op.n);
  ...
  if (!impFlag || impFlag == 1){
  	...
  	check_result<float>((float *)openmp_op.y, (float *)op.y, openmp_op.n);
  }
  ...
  */
  if (!impFlag || impFlag == 2){
  	...
  	cudaDeviceSynchronize();
  	check_result<float>((float *)cuSPARSE_op.y, (float *)op.y, cuSPARSE_op.n);
  }
  ...
  if (!impFlag || impFlag == 4){
  	...
  	bsr_op.cuSPARSE_bsr();
  	cudaDeviceSynchronize();
  	check_result<float>((float *)bsr_op.y, (float *)op.y, bsr_op.n);
  }
