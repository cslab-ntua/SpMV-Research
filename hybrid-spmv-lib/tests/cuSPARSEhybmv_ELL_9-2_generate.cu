...
int main(int argc, char **argv) {
  ...
  cout << "Device [" <<  device_id << "] " << deviceProp.name << ", " << " @ " << deviceProp.clockRate * 1e-3f << "MHz. " << endl;
	...
	fprintf(stdout,
	  "File=%s ( distribution = %s, placement = %s, diagonal_factor = %lf, seed = %d ) -> Generate time=%lf s\n\t\
  ...
  fprintf(stdout,"\nRunning tests.. \n");
	...
	fprintf(stdout, "cuSPARSE_ell: t = %lf ms (%lf Gflops/s )\n", cuSPARSE_op.timer, gflops_s);
	...
	"," << "cuSPARSE_ell" <<  "," << cuSPARSE_op.timer << "," << gflops_s << "\n";
