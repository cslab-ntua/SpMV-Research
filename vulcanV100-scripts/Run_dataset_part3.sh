...
do
	for dtype in D #S
	do
		for filename in `cat ${SCRIPTDIR}/dataset_part3.in`;
		do
			...
			do
				...
				do
					$exec $STORE_TIMER_DIR/${system}_dtype-${dtype}_run_dataset_part3.csv $line &>>$STORE_TIMER_DIR/${system}_dtype-${dtype}_run_dataset_part3.err
				done
			done
		done
	done
